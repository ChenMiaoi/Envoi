...
Directory navigation uses actual PDF bookmark XYZ destinations, not guessed pages. `build/preview.json` records SHA-256 fingerprints of the compilation inputs and PDF. This hidden build artifact lets reopened projects verify that their PDF still matches the source. A unique matching bookmark is required; edits, mismatched/foreign PDFs, or absent destinations cause source-only navigation with a light explanation. The current successful in-memory result uses its exact compilation signature. This is bookmark-based one-way navigation, not SyncTeX or continuous scroll synchronization. `scripts/test-demo-navigation.mjs` verifies all 43 demo destinations and rejects edited source/replaced PDF.
...
Checks: `test-git-status.mjs`, `test-diagnostics.mjs`, `test-diagnostic-restore.mjs`, and `test-lint.mjs` exercise real local tools; repository unit tests cover parsing, navigation, persistence and stale-state safeguards. Any fixture staging is confined to a temporary test repository; the real demo remains uncommitted.
