...
General settings contain an entry to `/settings/global/shortcuts`. This separate page supports command search, scope filter, key capture, individual reset and reset all. Bindings are stored as canonical strings such as `mod+alt+enter = compile` inside browser preferences; `mod` matches both Ctrl and Command, and legacy per-command overrides migrate onto the default table. Conflicts, reserved browser/system/editor keys and modifier-less combinations are rejected. All commands use one registry for display and dispatch. The default save is Ctrl/Command+S and saves all dirty project files immediately, without waiting for blur or a timer and without compiling. Changing that binding changes the active save key; the browser's webpage-save action remains suppressed.

Default bindings layer modifiers by intent: Mod+Shift switches pages (reader, writer, library, settings), Mod+Alt acts inside the current page — reader tab close/previous/next plus directory tree and AI panel toggles, writer outline/references/assets panels — and bare Mod keeps save and the command palette. Page-scoped commands fire only on their own page. The history view and project manage/close commands are bindable but have no default chord. No experimental or unimplemented commands are registered.
