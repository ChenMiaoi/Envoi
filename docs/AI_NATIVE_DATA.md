...
Configured OpenCode Zen, OpenCode Go, OpenAI, Anthropic and Google connections request their model-list endpoints. OpenCode's public directory determines membership; it does not establish account permission. Authenticated lists likewise cannot guarantee future quota or inference success. Runtime protocol, context/output limits, modalities and explicit thinking effort values come from models.dev (also used by OpenCode), with fixed provider protocol adapters. Models without enough verified metadata are listed unavailable rather than assigned invented capabilities. The resulting supported entries replace that provider's Pi runtime registration, so discovery and actual session lookup share the same IDs. A removed current choice is preserved but unavailable; no automatic replacement or inference occurs.

Successful catalogs persist under ai/model-catalogs, keyed by provider and a credential fingerprint, without credentials in content. Memory and disk caches expire after five minutes. Opening the model menu automatically refreshes an empty or expired catalog for a configured provider, with concurrent requests deduplicated. Successful credential/OAuth configuration and the explicit model-menu refresh action fetch again. Refresh errors or anomalous empty lists preserve the same connection's last successful catalog, record its stale state, and expose the actual failure; changing credentials cannot reuse another credential's cache. Source and update time remain in the local cache; the menu shows only compact refresh/settings actions, loading state and actual errors.

Configured providers without a discovery adapter retain Pi's bundled SDK models. Refresh reloads this SDK list; it does not verify a live provider catalog or account access. The AI settings also offer an advanced manual compatible-provider/model form, and user-entered model IDs remain explicitly manual. Discovery performs read-only requests only. Sources: https://opencode.ai/docs/zen/ , https://opencode.ai/docs/go/ , https://opencode.ai/docs/models/ , https://models.dev/api.json , https://developers.openai.com/api/reference/resources/models/methods/list , https://platform.claude.com/docs/en/api/models/list , https://ai.google.dev/api/models .

Assistant messages use the existing react-markdown/GFM/math/KaTeX stack in both streaming and restored history. User text and provider errors remain plain text. Raw HTML is skipped, generated images are represented as text (no automatic fetch), links accept only HTTP(S)/mailto with noopener/noreferrer, and KaTeX trust is disabled. Code, tables and display math scroll internally rather than widening the fixed-height transcript. The local SSR regression checks partial stream fragments, semantic Markdown and unsafe content without inference.
