...
Markdown uses one CodeMirror document with continuous cursor movement, selection, paste and undo. The live editor itself is the rich preview: every inactive top-level block is rendered through the same GFM, math, callout and code-highlighting pipeline as the reader, while clicking a block exposes that block's authoritative Markdown source for editing. Original source is authoritative, and changes map editor positions back to original line endings. There is no separate read-only preview, HTML-to-Markdown round trip, paragraph-mode button or source-mode toolbar. Raw HTML/scripts are never executed. Relative images are resolved only against files already present in the selected project; external images are not loaded. Supported links remain interactive. The centered measure and preview text use reader typography, exposed source uses editor typography, and PDFs/images remain independent.
