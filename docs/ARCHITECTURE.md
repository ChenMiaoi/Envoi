...
`services/project-sessions.ts` owns project identities, active window roots, resource tokens, watcher tickets, pending operation reservations and paper-browser cancellation. Project switches invalidate the old watcher; superseded or closed project bindings cannot become active after asynchronous loading. Closing a project or destroying its window invalidates pending watcher attachment, disposes language sessions and cancels browsing and pending AI, lint, LSP and compile reservations. IPC and window adapters additionally cancel utility-worker tasks. Restricting a root cancels operations on its registered descendants. Deleting a root clears every active owner and revokes its resource token.
