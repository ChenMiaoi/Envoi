...
export const viewNames: Record<ViewId, string> = {
  reader: "阅读",
  writer: "写作",
  library: "论文库",
  history: "版本历史",
...
export function resolvePage(pathname: string): { view?: ViewId; redirect?: string } {
  ...
  if (settings) {
    if (!settings[2]) return { view: "settings", redirect: `/settings/${settings[1]}/general` }
    if (
      settings[2] in settingCategories ||
      (settings[1] === "global" && settings[2] === "shortcuts")
    )
      return { view: "settings", ...(normalized !== pathname ? { redirect: normalized } : {}) }
    return {}
  }
