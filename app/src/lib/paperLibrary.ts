...
export interface LibraryPaper {id:string;title:string;author:string;year:string;venue:string;tags:string[];collection:string;status:'待读'|'在读'|'已读';notes:string;created:number;attachment?:Blob;attachmentName?:string;bib?:string;citationKey?:string}
...
export function filterLibrary(papers:LibraryPaper[],query:string,collection:string,status:string,year=''){
 const terms=query.toLocaleLowerCase().trim().split(/\s+/).filter(Boolean);
 return papers.filter(paper=>(!collection||paper.collection===collection)&&(!status||paper.status===status)&&(!year||paper.year===year)&&terms.every(term=>[paper.title,paper.author,paper.year,paper.venue,...paper.tags,paper.citationKey??''].join(' ').toLocaleLowerCase().includes(term))).sort((a,b)=>b.created-a.created);
}
...
export async function importLibraryFiles(files:File[]):Promise<LibraryPaper[]>{
 const records:LibraryPaper[]=[];for(const file of files){
  ...
  if(/\.pdf$/i.test(file.name)){await validateLibraryPdf(file);records.push({...base,id:crypto.randomUUID(),title:file.name.replace(/\.pdf$/i,''),attachment:file,attachmentName:file.name});}
