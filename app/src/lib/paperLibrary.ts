...
export function createLibraryStore(name='paperdesk-library-v1'){
 ...
 return {
  ...
  async remove(id:string){const record=await nativeGet<unknown>('library');if(!record)throw Error(translate('library.migrateFirst'));const next=(decodeNative(record.value) as LibraryPaper[]).filter(item=>item.id!==id);await nativePut('library',await encodeNative(next),'default',{expectedRevision:record.revision});await cache.remove(id);}
 };
}
...
export async function validateLibraryPdf(file:Blob){if(file.size>100_000_000)throw Error(translate('library.pdfTooLarge'));if(!(await file.slice(0,1024).text()).includes('%PDF-'))throw Error(translate('library.notPdf'));}
export async function importLibraryFiles(files:File[]):Promise<LibraryPaper[]>{
 const records:LibraryPaper[]=[];for(const file of files){
  ...
  else if(/\.bib$/i.test(file.name)){if(file.size>5_000_000)throw Error(translate('library.bibTooLarge'));for(const entry of parseBibliography(await file.text()))records.push({...base,id:crypto.randomUUID(),title:entry.title===translate('bib.noTitle')?'':entry.title,author:entry.author===translate('bib.noAuthor')?'':entry.author,year:entry.year,venue:entry.venue,bib:entry.raw,citationKey:entry.key});}
  else throw Error(translate('library.importUnsupported',{name:file.name}));
 }return records;
}
export function libraryAttachment(paper:LibraryPaper){if(!paper.attachment)throw Error(translate('library.noAttachment'));return new File([paper.attachment],paper.attachmentName??`${paper.title||'paper'}.pdf`,{type:'application/pdf'});}
