...
async function gitBridge<T>(call:()=>Promise<unknown>,failure:string):Promise<T> {
 ...
 const result=await call().catch(error=>{throw ipcError(error);}) as {ok?:boolean;error?:string}|null;
...
export async function initializeLocalGit(rootPath:string) {
 ...
 for(const configPath of [`${managementDirName}/project.json`,`${legacyDirName}/project.json`,'paperdesk.json']) {
  const content=await envoi().fsRead(rootPath,configPath).catch(()=>undefined);
  if(content?.text===undefined)continue;
  const json=JSON.parse(content.text);json.git={...json.git,status:'initialized'};
