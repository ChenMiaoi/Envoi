...
async function gitBridge<T>(directory:FileSystemDirectoryHandle,path:string,endpoint:string,failure:string,extra:Record<string,unknown>={}):Promise<T> {
 ...
 try{const response=await fetch(`/api/paperdesk/${endpoint}`,{method:'POST',headers:{'Content-Type':'application/json','X-PaperDesk-Token':runtime.token},body:JSON.stringify({directory:path,proof,...extra})});if(!response.headers.get('content-type')?.includes('application/json'))throw Error('本地 Git 桥接不可用。');const result=await response.json();if(!response.ok||!result.ok)throw Error(result.error??failure);return result as T;}finally{await state.removeEntry('git-proof').catch(()=>{});}
}
...
export async function bindProjectConnection(directory:FileSystemDirectoryHandle,path:string) {
 await gitBridge(directory,path,'project-bind','项目连接校验失败');
