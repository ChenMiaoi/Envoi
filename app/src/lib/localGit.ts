...
async function gitBridge<T>(directory:FileSystemDirectoryHandle,path:string,endpoint:string,failure:string,extra:Record<string,unknown>={}):Promise<T> {
 ...
 const state=await directory.getDirectoryHandle(managementDirName,{create:true}).catch(error=>{if(error.name==='NotFoundError')throw Error('项目目录已移动或不存在，请在项目菜单打开新位置；这不是 Git 授权失效。');throw error;});
 ...
 try{const response=await fetch(`/api/envoi/${endpoint}`,{method:'POST',headers:{'Content-Type':'application/json','X-Envoi-Token':runtime.token},body:JSON.stringify({directory:path,proof,proofKind:'unique',...extra})});if(!response.headers.get('content-type')?.includes('application/json'))throw Error('本地 Git 桥接不可用。');const result=await response.json();if(!response.ok||!result.ok)throw Error(result.error??failure);return result as T;}finally{await state.removeEntry('git-proof-'+proof).catch(()=>{});}
}
