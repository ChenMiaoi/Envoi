import {rememberGitPath} from './gitBinding';
export async function localGitRuntime() {
 const response=await fetch('/api/paperdesk/git');
 if(!response.ok||!response.headers.get('content-type')?.includes('application/json'))throw new Error('本地 Git 服务未连接；无法检测安装状态。请启动本地服务或取消启用 Git。');
 return response.json() as Promise<{available:boolean;version?:string;error?:string;token:string}>;
}
export async function initializeLocalGit(directory:FileSystemDirectoryHandle, absolutePath:string) {
 const runtime=await localGitRuntime();if(!runtime.available)throw new Error(runtime.error);
 const proof=Array.from(crypto.getRandomValues(new Uint8Array(32)),byte=>byte.toString(16).padStart(2,'0')).join('');
 const state=await directory.getDirectoryHandle('.paperdesk',{create:true}).catch(error=>{if(error.name==='NotFoundError')throw Error('项目目录已移动或不存在，请在项目菜单打开新位置；这不是 Git 授权失效。');throw error;});
 const handle=await state.getFileHandle('git-proof',{create:true});const stream=await handle.createWritable();await stream.write(proof);await stream.close();
 try {
  const response=await fetch('/api/paperdesk/git-init',{method:'POST',headers:{'Content-Type':'application/json','X-PaperDesk-Token':runtime.token},body:JSON.stringify({directory:absolutePath,proof})});
  const result=await response.json();if(!response.ok||!result.ok)throw new Error(result.error??'Git 初始化失败');
  await rememberGitPath(directory,absolutePath);
  let metadata:FileSystemFileHandle;try{metadata=await state.getFileHandle('project.json');}catch(error){if((error as Error).name!=='NotFoundError')throw error;metadata=await directory.getFileHandle('paperdesk.json');}const json=JSON.parse(await (await metadata.getFile()).text());json.git.status='initialized';const writable=await metadata.createWritable();await writable.write(JSON.stringify(json,null,2)+'\n');await writable.close();
 }finally{await state.removeEntry('git-proof').catch(()=>{});}
}
export interface GitStatus {state:'ready'|'not-initialized';branch?:string;detached?:boolean;version:string;files:{path:string;originalPath?:string;index:string;worktree:string;untracked:boolean;conflict:boolean}[]}
export async function localGitStatus(directory:FileSystemDirectoryHandle,path:string):Promise<GitStatus> {
 const runtime=await localGitRuntime();if(!runtime.available)throw Error(runtime.error);
 if(await directory.queryPermission({mode:'readwrite'})!=='granted')throw Error('目录权限已失效，请在项目菜单恢复授权。');
 const proof=Array.from(crypto.getRandomValues(new Uint8Array(32)),b=>b.toString(16).padStart(2,'0')).join('');
 const state=await directory.getDirectoryHandle('.paperdesk',{create:true}).catch(error=>{if(error.name==='NotFoundError')throw Error('项目目录已移动或不存在，请在项目菜单打开新位置；这不是 Git 授权失效。');throw error;});const handle=await state.getFileHandle('git-proof',{create:true});const stream=await handle.createWritable();await stream.write(proof);await stream.close();
 try{const response=await fetch('/api/paperdesk/git-status',{method:'POST',headers:{'Content-Type':'application/json','X-PaperDesk-Token':runtime.token},body:JSON.stringify({directory:path,proof})});if(!response.headers.get('content-type')?.includes('application/json'))throw Error('本地 Git 桥接不可用。');const result=await response.json();if(!response.ok||!result.ok)throw Error(result.error??'Git 状态读取失败');return result;}finally{await state.removeEntry('git-proof').catch(()=>{});}
}
export async function bindProjectConnection(directory:FileSystemDirectoryHandle,path:string) {
 const runtime=await localGitRuntime();
 if(await directory.queryPermission({mode:'readwrite'})!=='granted')throw Error('项目目录权限需在打开项目时恢复。');
 const proof=Array.from(crypto.getRandomValues(new Uint8Array(32)),b=>b.toString(16).padStart(2,'0')).join('');
 const state=await directory.getDirectoryHandle('.paperdesk',{create:true}).catch(error=>{if(error.name==='NotFoundError')throw Error('项目目录已移动或不存在，请在项目菜单打开新位置；这不是 Git 授权失效。');throw error;});const handle=await state.getFileHandle('git-proof',{create:true});const stream=await handle.createWritable();await stream.write(proof);await stream.close();
 try{const response=await fetch('/api/paperdesk/project-bind',{method:'POST',headers:{'Content-Type':'application/json','X-PaperDesk-Token':runtime.token},body:JSON.stringify({directory:path,proof})});const result=await response.json();if(!response.ok||!result.ok)throw Error(result.error??'项目连接校验失败');await rememberGitPath(directory,path);}finally{await state.removeEntry('git-proof').catch(()=>{});}
 window.dispatchEvent(new Event('paperdesk:connection-updated'));
}
