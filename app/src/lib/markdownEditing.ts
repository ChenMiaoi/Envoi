...
export function replaceMarkdownBlock(source: string, start: number, end: number, text: string) {
  if (start < 0 || end < start || end > source.length)
    throw Error(translate("editor.invalidMarkdownRange"))
...
export function resolveProjectLink(from: string, url: string) {
  ...
  if (decoded.includes("\\") || [...decoded].some((character) => character.charCodeAt(0) < 32))
    return null
...
export function editMarkdownChanges(
  source: string,
  ...
  for (const change of [...changes].sort((a, b) => b.from - a.from)) {
    const from = offsets[change.from],
      to = offsets[change.to]
    if (from === undefined || to === undefined)
      throw Error(translate("editor.invalidMarkdownRange"))
    next = next.slice(0, from) + change.insert.replace(/\r\n|\r|\n/g, ending) + next.slice(to)
  }
  return next
}
