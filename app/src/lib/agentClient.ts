...
export async function bindProject(rootPath: string, options: { copy?: boolean } = {}) {
  ...
  const promise = (async () => {
    const result = await envoi()
      .bindProject(rootPath, { copy })
      .catch((error) => {
        throw ipcError(error)
      })
    if (result.ok)
      await rememberGitPath(result.project.id, result.project.path).catch((error) => {
...
export async function* agentChat(
  ...
  const unsubscribe = envoi().onAgentEvent((event) => {
    ...
    const rest = { ...event }
    delete (rest as Partial<typeof event>).projectId
  ...
  const abort = () => {
    void agentRequest("abort", { projectId: options.projectId, sessionId }).catch(() => {})
    push({ end: true, error: new DOMException("Aborted", "AbortError") })
  }
  options.signal?.addEventListener("abort", abort, { once: true })
  try {
    if (options.signal?.aborted) throw new DOMException("Aborted", "AbortError")
    const start = await envoi()
      .agentChat({
...
export function refreshModelCatalog(provider: string) {
  const pending = catalogRefreshes.get(provider)
  if (pending) return pending
  const request = agentRequest("models/refresh", { provider }).finally(() => {
    catalogRefreshes.delete(provider)
  })
  catalogRefreshes.set(provider, request)
  return request
}
