...
async function lookupDoi(doi:string):Promise<Enrichment|null>{
 ...
 let bib:string|undefined;try{const response=await fetch(`https://api.crossref.org/works/${encodeURIComponent(doi)}/transform/application/x-bibtex`,{headers});if(response.ok)bib=(await response.text()).trim();}catch{/* BibTeX transform is best-effort; fields alone still enrich. */}
...
async function lookupArxiv(id:string):Promise<Enrichment|null>{
 ...
 const data=(await fetchJson(`https://api.datacite.org/dois/${encodeURIComponent(doi)}`)).data?.attributes;
 const fields=dataciteToPaper(data);if(!fields)return null;
 let bib:string|undefined;try{const response=await fetch(`https://api.datacite.org/dois/${encodeURIComponent(doi)}`,{headers:{...headers,Accept:'application/x-bibtex'}});if(response.ok)bib=(await response.text()).trim();}catch{/* best-effort */}
 return {fields,bib,citationKey:bib?bibtexKey(bib):undefined,source:`DataCite arXiv ${id}`};
}
...
async function searchTitle(hint:string):Promise<Enrichment|null>{
 const data=await fetchJson(`https://api.crossref.org/works?query.title=${encodeURIComponent(hint)}&rows=5`);
...
interface PdfScan{text:string;titleHint:string;xmp:Record<string,unknown>;info:{Title?:string;Author?:string}}
async function scanPdf(blob:Blob,maxPages=2):Promise<PdfScan>{
 const task=getDocument({data:await blob.arrayBuffer()});
 try{
  const document=await task.promise;const parts:string[]=[];const lines:{text:string;fontSize:number}[]=[];
  for(let page=1;page<=Math.min(document.numPages,maxPages);page++){
   const content=await(await document.getPage(page)).getTextContent();
   parts.push(content.items.map(item=>('str' in item?item.str:'')).join(' '));
   if(page===1)for(const item of content.items){if(!('str' in item)||!item.str.trim())continue;lines.push({text:item.str,fontSize:Math.hypot(item.transform[2],item.transform[3])});}
  }
  const xmp:Record<string,unknown>={};let info:{Title?:string;Author?:string}={};
  try{const meta=await document.getMetadata();for(const key of ['dc:title','dc:creator','prism:coverDate','prism:publicationDate','dc:date','prism:publicationName','prism:doi']){const value=meta.metadata?.get(key);if(value!=null&&value!=='')xmp[key]=value;}info=meta.info as {Title?:string;Author?:string};}catch{/* metadata optional */}
  return {text:parts.join('\n'),titleHint:pickTitleHint(lines),xmp,info};
 }finally{void task.destroy();}
}
export async function extractPdfText(blob:Blob,maxPages=2){return (await scanPdf(blob,maxPages)).text;}
...
export async function enrichPaper(hint:{attachment?:Blob;title:string}):Promise<Enrichment|null>{
 let scan:PdfScan|null=null;
 if(hint.attachment){try{scan=await scanPdf(hint.attachment);}catch{return null;}}
 try{
  if(scan){
  ...
  if(arxiv){const found=await lookupArxiv(arxiv);if(found)return found;}
