...
export function rememberGitPath(projectId: string, path: string) {
  return updateBindings((current) => ({ ...current, [projectId]: path }))
}
// 删除流程：找出根路径等于 rootPath 或位于其下的所有绑定项目。
export async function matchingGitBindings(rootPath: string) {
  const root = rootPath.replace(/\/$/, ""),
    bindings = await nativeGet<Record<string, string>>("bindings")
  return Object.entries(bindings?.value ?? {})
    .filter(([, path]) => nativePathWithin(path, root))
    .map(([id]) => id)
}
export function forgetGitBindings(projectIds: string[]) {
  return updateBindings((current) =>
    Object.fromEntries(Object.entries(current).filter(([id]) => !projectIds.includes(id))),
  )
}
