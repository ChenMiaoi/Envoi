...
export interface ProjectFile { version?: string; file?: File; id: string; path: string; kind: FileKind; text?: string; saved?: string; url?: string }
export interface PaperProject { settings?:ProjectConfiguration; lint?: {fileId:string;text:string;status:"checking"|"ready"|"unavailable"|"disabled";message?:string;items:Diagnostic[]}; diagnostics?: CompileDiagnostics; engine?: "pdflatex" | "xelatex"; compiled?: { file: File; signature: string; synctex?: Uint8Array<ArrayBuffer> }; compileStatus?: string; compileLog?: string; id: string; name: string; files: ProjectFile[]; directories: string[]; rootId: string; rootPath?: string }
...
const skippedParts: Record<string, true> = { ".git": true, [managementDirName]: true, [legacyDirName]: true, node_modules: true };
export async function readProject(rootPath: string): Promise<PaperProject> {
  ...
  for (const path of listing.directories) if (!path.split("/").some((part) => skippedParts[part])) directories.push(path);
  for (const entry of listing.files) {
    ...
    if (isTextPath(entry.path) && text !== undefined && text.length > 5_000_000) throw new Error(translate('project.textTooLarge',{path: entry.path}));
    if (text?.includes("\0")) text = undefined;
    files.push({ id: entry.path, path: entry.path, kind: fileKind(entry.path), version: entry.version, text, saved: text });
  }
  const config = await projectConfigFile(rootPath);
  if (config) files.push({ id: config.path, path: config.path, kind: 'text', text: config.text, saved: config.text });
  ...
  const withUrls = await Promise.all(files.map(async (file) => file.text === undefined ? { ...file, url: await envoi().assetUrl(rootPath, file.path).then(url => file.version === undefined ? url : url + '?v=' + encodeURIComponent(file.version)).catch(() => undefined) } : file));
  const project:PaperProject = { settings:projectConfiguration(metadata.settings,metadata.engine),diagnostics,compileLog:diagnostics?.log,compileStatus:diagnostics?translate('compile.restoredRecord',{status:translate(diagnostics.status==='success'?'compile.statusSuccess':diagnostics.status==='failed'?'compile.statusFailed':'compile.statusCancelled')}):undefined,engine: ["pdflatex", "xelatex"].includes(metadata.engine ?? "") ? metadata.engine : undefined, id: listing.projectId??metadata.projectId??crypto.randomUUID(), name: listing.name??nativeBasename(rootPath), rootPath, files: withUrls, directories, rootId: root?.id ?? "" };
  if(!diagnostics){
    const log=files.find(file=>file.path==='build/compile.log'),pdf=project.files.find(file=>file.path==='build/main.pdf');
    if(log?.text&&pdf){const verified=await verifyPreview(project,pdf);diagnostics={items:parseDiagnostics(log.text,files),signature:verified?projectSignature(project):'legacy-unverified',rootId:project.rootId,status:'success',log:log.text,engine:project.engine};project.diagnostics=diagnostics;project.compileLog=log.text;project.compileStatus=translate('compile.restoredDisk');}
  }
  return project;
}
export async function createTextFile(rootPath: string, path: string, text: string) {
  const parts = safePath(path);
  const exists = await envoi().fsRead(rootPath, path).then(() => true, () => false);
  if (exists) throw new Error(translate('project.fileExists',{path}));
  for (let depth = 1; depth < parts.length; depth++) await envoi().fsMkdir(rootPath, parts.slice(0, depth).join("/"));
  const result = await envoi().fsSave(rootPath, [{path, text, expectedText: null}]);
  if (result.error) throw new Error(result.error);
}
export async function createPaper(parentRootPath: string, name: string, template = "article", enableGit = true): Promise<string> {
  ...
  if (listing.some(entry => entry.name === name)) throw new Error(translate('project.directoryExists',{name}));
  const parent = parentRootPath.replace(/\/+$/, ""), rootPath = `${parent}/${name}`;
  try {
    const templates = Object.entries(templateFiles(template, enableGit));
    const folders = [...new Set(templates.flatMap(([path]) => safePath(path).slice(0, -1).map((_, depth, parts) => `${name}/${parts.slice(0, depth + 1).join("/")}`)))];
    await envoi().fsMkdir(parent, name);
    for (const folder of folders) await envoi().fsMkdir(parent, folder);
    await envoi().fsWriteFiles(parent, templates.map(([path, text]) => ({ path: `${name}/${path}`, text })));
...
export async function saveProject(project: PaperProject, onSaved: (id: string, text: string) => void) {
  ...
  const result = await envoi().fsSave(project.rootPath, changes.map(file => ({path: file.path, text: file.text!, expectedText: file.saved ?? null})));
...
export async function persistBuild(rootPath: string, pdf: File, log: string, manifest?: string, synctex?: Uint8Array<ArrayBuffer>) {
...
export function mergeDiskProject(current: PaperProject, disk: PaperProject): PaperProject {
  ...
  const files = disk.files.map(file => {const draft = drafts.get(file.path); drafts.delete(file.path); return draft ?? file;});
  files.push(...drafts.values());
  files.sort((a, b) => a.path.localeCompare(b.path));
  const same = files.length === current.files.length && files.every((file, index) => {
    const previous = current.files[index];
    return file.path === previous.path && file.text === previous.text && file.saved === previous.saved && (file.text !== undefined || file.version === previous.version);
  });
  if (same && JSON.stringify(disk.directories) === JSON.stringify(current.directories) && JSON.stringify(disk.settings) === JSON.stringify(current.settings)) return current;
