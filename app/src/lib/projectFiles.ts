...
export async function readProject(rootPath: string): Promise<PaperProject> {
  ...
  const project:PaperProject = { settings:projectConfiguration(metadata.settings,metadata.engine),diagnostics,compileLog:diagnostics?.log,compileStatus:diagnostics?translate('compile.restoredRecord',{status:translate(diagnostics.status==='success'?'compile.statusSuccess':diagnostics.status==='failed'?'compile.statusFailed':'compile.statusCancelled')}):undefined,engine: ["pdflatex", "xelatex"].includes(metadata.engine ?? "") ? metadata.engine : undefined, id: metadata.projectId??crypto.randomUUID(), name: rootPath.split("/").filter(Boolean).pop() ?? rootPath, rootPath, files: withUrls, directories, rootId: root?.id ?? "" };
  if(!diagnostics){
