...
export function extractIdentifiers(text: string): { doi?: string; arxiv?: string } {
  ...
  const doi = /\b10\.\d{4,9}\/[-._;()/:A-Z0-9]+/i
    .exec(text.replace(/-\s+/g, "-"))?.[0]
    .replace(/[.,;:)\]}>’”-]+$/, "")
    .replace(/[<([{’“]+$/, "")
  const arxiv =
    /(?:arxiv:\s*|arxiv\.org\/(?:abs|pdf)\/)(\d{4}\.\d{4,5})(?:v\d+)?/i.exec(text)?.[1] ??
    /\b(\d{4}\.\d{4,5})(?:v\d+)?\b/.exec(text)?.[1]
  return { doi, arxiv }
}
// Publisher-embedded PDF metadata: XMP packet values (pdf.js getMetadata().metadata.getAll()) plus the Info dictionary.
export function xmpToPaper(
  xmp: Record<string, unknown>,
  info: { Title?: string; Author?: string },
): { fields: Partial<Pick<LibraryPaper, "title" | "author" | "year" | "venue">>; doi?: string } {
  const text = (value: unknown) => (typeof value === "string" ? value.trim() : "")
  const creators = xmp["dc:creator"]
  const author = Array.isArray(creators)
    ? creators.map(text).filter(Boolean).join("; ")
    : text(creators)
  const year =
    /\d{4}/.exec(
      text(xmp["prism:coverDate"]) || text(xmp["prism:publicationDate"]) || text(xmp["dc:date"]),
    )?.[0] ?? ""
  return {
    fields: {
      title: text(xmp["dc:title"]) || info.Title?.trim() || "",
      author: author || info.Author?.trim() || "",
      year,
      venue: text(xmp["prism:publicationName"]),
    },
    doi: text(xmp["prism:doi"]) || undefined,
  }
}
...
export function pickTitleHint(lines: { text: string; fontSize: number }[]): string {
  const usable = lines.filter(
    (line) => line.text.trim().length > 1 && !TITLE_NOISE.test(line.text.trim()),
  )
  const max = Math.max(...usable.map((line) => line.fontSize), 0)
  if (max <= 0) return ""
...
export function dataciteToPaper(
  data:
    | {
        titles?: { title: string }[]
    ...
    | undefined,
): Pick<LibraryPaper, "title" | "author" | "year" | "venue"> | null {
  const title = data?.titles?.[0]?.title?.replace(/\s+/g, " ").trim()
...
export function crossrefToPaper(
  work: CrossrefWork,
): Pick<LibraryPaper, "title" | "author" | "year" | "venue"> {
  return {
...
export function synthesizeBib(
  paper: Pick<LibraryPaper, "title" | "author" | "year" | "venue">,
  key: string,
): string {
  const fields = [
    ["author", paper.author.replace(/;\s*/g, " and ")],
    ["title", paper.title],
    ["year", paper.year],
    ["journal", paper.venue],
  ].filter(([, v]) => v)
...
export function exportBibliography(papers: LibraryPaper[]): { text: string; skipped: number } {
  ...
  for (const paper of papers) {
    ...
    if (!bib) {
      ...
      try {
        key ??= citationKeyFor(paper)
      } catch {
        skipped++
        continue
      }
      bib = synthesizeBib(paper, key)
    }
    if (key && seen.has(key)) {
      skipped++
      continue
