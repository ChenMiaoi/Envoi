...
export function parseBibliography(text: string) {
  ...
  return library.entries.map((entry) => {
    if (!entry.key || /[\s,{}\\%]/.test(entry.key)) throw new Error(translate('bib.unsupportedKey',{key:entry.key}));
    ...
    return { key: entry.key, title: plain(entry.fields.title) || translate('bib.noTitle'), author: entry.fields.author?.map((author) => author.name || [author.firstName, author.prefix, author.lastName, author.suffix].filter(Boolean).join(" ")).join("; ") || translate('bib.noAuthor'), year: entry.fields.year || entry.fields.date || "", venue: plain(entry.fields.journal || entry.fields.booktitle), raw: entry.input };
  });
}
