...
export async function deleteVerifiedProject(plan:DeletionTarget,typed:string){
 ...
 try{await envoi().fsRemoveTree(plan.path);}catch(error){throw Error(translate('project.deleteIncomplete',{message:(error as Error).message}));}
 const gone=await envoi().fsList(plan.path).then(()=>false,()=>true);
