...
export class PdfRenderScheduler {
  ...
  enqueue(run: (signal: AbortSignal) => Promise<void>) {
    const controller = new AbortController()
    this.jobs.push({ controller, run })
    this.pump()
    return () => {
      controller.abort()
      this.jobs = this.jobs.filter((job) => job.controller !== controller)
    }
  }
  private pump() {
    ...
    void job
      ...
      .catch(() => {
        /* The page reports rendering errors. */
      })
      .finally(() => {
