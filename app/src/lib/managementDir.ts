...
export async function projectConfigFile(rootPath: string): Promise<{ path: string; text: string } | undefined> {
  for (const name of [managementDirName, legacyDirName]) {
    const result = await envoi().fsRead(rootPath, `${name}/project.json`).catch(() => undefined);
