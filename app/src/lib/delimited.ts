  ...
  while (i < source.length) {
    ...
    if (source[i] === '"') {
      ...
      while (
        ...
        if (source[i] === '"')
          throw Error(translate("csv.quoteInUnquoted", { line: rows.length + 1 }))
        value += source[i++]
      }
    }
    ...
    if (source[i] === delimiter) {
      i++
      if (i === source.length) row.push({ start: i, end: i, value: "", quoted: false })
      else continue
...
export function editDelimitedCell(
  source: string,
  cell: DelimitedCell,
  value: string,
  delimiter = ",",
  ...
  const encoded =
    cell.quoted || value.includes(delimiter) || /["\r\n]/.test(value)
      ? `"${value.replaceAll('"', '""')}"`
      : value
