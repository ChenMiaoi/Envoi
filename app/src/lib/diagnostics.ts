...
export function parseDiagnostics(log:string,files:Pick<ProjectFile,'path'|'text'>[],failed=false):Diagnostic[]{
 ...
 for(let i=0;i<lines.length;i++){
  ...
  for(const match of line.matchAll(/\(([^\s()]+\.(?:tex|bib|sty|cls))?|\)/g)){if(match[0]===')')stack.pop();else stack.push(match[1]?mapPath(match[1]):undefined);}
  const current=/(?:Warning:|^Warning--|^!|^(?:Overfull|Underfull))/.test(line)?priorFile??[...stack].reverse().find(Boolean):[...stack].reverse().find(Boolean);
  const located=/^(.+?\.(?:tex|bib|sty|cls)):(\d+):\s*(.+)$/.exec(line);
  if(located){if(/Fatal error occurred/.test(located[3])&&items.some(item=>item.severity==='error'))continue;add('error',located[3],mapPath(located[1]),Number(located[2]));continue;}
  if(/^!\s/.test(line)){const next=lines.slice(i+1,i+8).join('\n'),position=/^l\.(\d+)/m.exec(next);add('error',line.replace(/^!\s*/,''),current,position?Number(position[1]):undefined);continue;}
  if(/(?:LaTeX|Package \S+|Class \S+) Warning:|^Warning--|^(?:Overfull|Underfull) \\[hv]box/.test(line)){
   let message=line;
   for(let j=i+1;j<Math.min(i+5,lines.length);j++){const next=lines[j];if(!next.trim()||/Warning:|^\[|^\(|^\)|^!|^<|^Output/.test(next))break;if(/^(?:on |n input|input |at lines|[a-z].*\.$)/.test(next)||/input line|--line \d+/.test(next))message+=(message.length>=70?'':' ')+next.trim();else break;}
   const position=/(?:input line|at lines?|--line)\s+(\d+)/.exec(message),bibFile=/of file\s+(\S+\.bib)/.exec(message);
...
export function diagnosticLocation(item:Diagnostic,files:ProjectFile[]){
 const file=files.find(file=>file.path===item.path);if(!file||file.text===undefined||!item.line)return undefined;
 const lines=file.text.split('\n');if(item.line<1||item.line>lines.length)return undefined;
 const start=lines.slice(0,item.line-1).reduce((n,line)=>n+line.length+1,0);return {fileId:file.id,path:file.path,start,end:start+lines[item.line-1].length,severity:item.severity};
}
