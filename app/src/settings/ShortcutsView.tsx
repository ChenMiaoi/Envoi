...
export function ShortcutsView(){
 ...
 const {t}=useT();
 const scopeNames:Record<Scope,string>={global:t('settings.shortcuts.scopeGlobal'),reader:t('command.view-reader'),writer:t('command.view-writer')};
 ...
 const reset=(command:Command)=>{
  ...
  if(!fallback){update({bindings:next});setMessage(t('settings.shortcuts.resetNoDefault'));return;}
  const invalid=validateChord(parseBinding(fallback)!.chord,next);
  if(invalid){setMessage(t('settings.shortcuts.defaultConflict',{reason:invalid}));return;}
 ...
 return <div className="h-full overflow-auto p-7"><div className="mx-auto max-w-3xl">
  <Link className="text-xs text-primary" to="/settings/global/general">{t('settings.shortcuts.back')}</Link>
  ...
  <div className="my-5 flex gap-3">
   <input aria-label={t('settings.shortcuts.searchAria')} placeholder={t('settings.shortcuts.searchPlaceholder')} value={query} onChange={e=>setQuery(e.target.value)} className="min-w-0 flex-1 rounded border border-input bg-card p-2 text-xs" />
   <select aria-label={t('settings.shortcuts.scopeAria')} value={scope} onChange={e=>setScope(e.target.value)} className="rounded border border-input bg-card p-2 text-xs">
   ...
   <button className="text-xs text-primary" onClick={()=>{update({bindings:[]});setRecording(null);setMessage(t('settings.shortcuts.allReset'));}}>{t('settings.shortcuts.resetAll')}</button>
  </div>
   ...
   {visible.map(item=>{
    ...
    return <div key={item.id} className="flex flex-wrap items-center justify-between gap-4 border-b border-border py-4 last:border-0">
     ...
     <div className="flex items-center gap-3">
      {recording===item.id
       ?<input data-shortcut-recorder autoFocus readOnly aria-label={t('settings.shortcuts.recordAria',{label:item.label})} placeholder={t('settings.shortcuts.recordPlaceholder')} className="w-52 rounded border border-primary bg-background p-2 text-xs" onKeyDown={event=>{
       ...
       :<button className="rounded border border-border px-3 py-2 text-xs" aria-label={t('settings.shortcuts.modifyAria',{label:item.label})} onClick={()=>{setMessage('');setRecording(item.id);}}><kbd>{chord?chordLabel(chord,mac):t('settings.shortcuts.unbound')}</kbd></button>}
      <button className="text-xs text-muted-foreground" onClick={()=>reset(item)}>{t('settings.shortcuts.reset')}</button>
     </div>
    </div>;
   })}
  </div>
