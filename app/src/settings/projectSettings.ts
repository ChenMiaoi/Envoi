import {projectConfiguration,type ProjectConfiguration} from './model';import type {PaperProject} from '@/lib/projectFiles';import type {AiConfig} from '@/lib/agentClient';
...
export async function saveProjectConfiguration(project:PaperProject,configuration:ProjectConfiguration,main=project.rootId,ai?:Partial<AiConfig>):Promise<PaperProject>{
 ...
 const readDisk=async(path:string)=>(await envoi().fsRead(project.rootPath!,path).catch(()=>undefined))?.text;
 let raw:string|undefined;
 for(const dir of known?[known.path.split('/')[0]]:[managementDirName,legacyDirName]){
  raw=await readDisk(`${dir}/project.json`);if(raw!==undefined)break;
 }
 if(known?raw!==known.saved:raw!==undefined)throw Error(translate('settings.project.externallyModified'));
 let metadata:Record<string,unknown>={};
 if(raw===undefined&&legacy){const legacyRaw=await readDisk('paperdesk.json');if(legacyRaw!==legacy.saved)throw Error(translate('settings.project.legacyModified'));const old=JSON.parse(legacyRaw!);for(const key of ['name','main','template','dataStatus','buildDirectory'])if(old[key]!==undefined)metadata[key]=old[key];if(old.git&&typeof old.git==='object')metadata.git={requested:old.git.requested,branch:old.git.branch,status:old.git.status};}
 ...
 const ignoreKnown=project.files.find(file=>file.path==='.gitignore');const ignoreRaw=await readDisk('.gitignore')??'';
 ...
 if(updateIgnore){const ignoreText=ignoreRaw+(ignoreRaw.endsWith('\n')||!ignoreRaw?'':'\n')+'\n# Share project settings; keep machine bindings and caches private\n'+ignoreRules+'\n';try{await envoi().fsWrite(project.rootPath,'.gitignore',{text:ignoreText});}catch(error){throw Error(translate('settings.project.gitignoreUpdateFailed',{error:(error as Error).message}));}ignoreFile={id:'.gitignore',path:'.gitignore',kind:'text',text:ignoreText,saved:ignoreText};}
 // Legacy files remain an untouched backup, including unknown fields. New configuration is authoritative.
 const file={id:projectConfigPath,path:projectConfigPath,kind:'text' as const,text,saved:text};
