...
export async function saveProjectConfiguration(project:PaperProject,configuration:ProjectConfiguration,main=project.rootId):Promise<PaperProject>{
 ...
 const root=project.files.find(file=>file.id===main&&file.kind==='latex');if(!root)throw Error('主文件必须是当前项目内的 LaTeX 文件。');
 ...
 const settings=projectConfiguration(configuration);metadata.settings=settings;metadata.main=root.path;delete metadata.engine;
 ...
 return {...project,settings,engine:settings.overrides.engine,rootId:root.id,files:[...project.files.filter(existing=>existing.path!==projectConfigPath&&existing.path!=='.gitignore'),file,...(ignoreFile?[ignoreFile]:[])]};
}
