import { appendChatEvent } from "@/lib/chatActivity.mjs"
import { useProjectTrust } from "@/project/useProjectTrust"
import { translate } from "@/i18n/runtime"
import { useCallback, useEffect, useRef, useState, type ReactNode } from "react"
import { AgentContext } from "./context"
import {
  agentStatus,
  agentRequest,
  agentChat,
  bindProject,
  type AgentStatus,
  type AgentRecord,
  type AiConfig,
} from "@/lib/agentClient"
import { useProject } from "@/project/context"
import { readProject, dirtyFiles } from "@/lib/projectFiles"
import { mergeDrafts } from "@/lib/projectSession"
interface ScopeState {
  record: AgentRecord | null
  sessions: AgentRecord[]
  busy: boolean
  error: string
  ready: boolean
  config: AiConfig | null
  root?: string
  name?: string
}
const blank: ScopeState = {
  record: null,
  sessions: [],
  busy: false,
  error: "",
  ready: false,
  config: null,
}
export function AgentProvider({ children }: { children: ReactNode }) {
  const { project, setProject, setAgentBusy, saveAll, busy: projectBusy } = useProject(),
    [status, setStatus] = useState<AgentStatus | null>(null),
    [scopes, setScopes] = useState<Record<string, ScopeState>>({})
  const trusted = useProjectTrust(project.rootPath)?.trusted
  const scope = project.id,
    current = scopes[scope] ?? blank,
    controllers = useRef(new Map<string, AbortController>()),
    navigation = useRef(new Set<string>()),
    [navigating, setNavigating] = useState(false)
  const patch = useCallback(
    (id: string, update: Partial<ScopeState> | ((state: ScopeState) => ScopeState)) =>
      setScopes((all) => ({
        ...all,
        [id]:
          typeof update === "function"
            ? update(all[id] ?? blank)
            : { ...(all[id] ?? blank), ...update },
      })),
    [],
  )
  const refresh = useCallback(async () => {
    const id = scope
    if (project.rootPath && !trusted) {
      patch(id, { ...blank })
      return
    }
    try {
      const status = await agentStatus()
      setStatus(status)
      if (id === "empty") {
        patch(id, { ...blank })
        return
      }
      if (id !== "global") {
        if (!project.rootPath) throw Error(translate("ai.projectNotConnected"))
        const result = await bindProject(project.rootPath)
        if (result.project.id !== id) {
          const fresh = await readProject(project.rootPath)
          setProject((current) => (current.id === id ? mergeDrafts(fresh, current) : current))
          return
        }
      }
      const result = await agentRequest<{
        sessions: AgentRecord[]
        settings: AiConfig
        activeId?: string
      }>("sessions", { projectId: id })
      const selected = result.activeId ?? result.sessions[0]?.id
      const record = selected
        ? await agentRequest<AgentRecord>("session", { projectId: id, sessionId: selected })
        : null
      patch(id, (state) => ({
        ...state,
        ready: true,
        error: "",
        config: result.settings,
        sessions: state.busy ? state.sessions : result.sessions,
        record: state.busy ? state.record : record,
      }))
    } catch (error) {
      patch(id, {
        ready: false,
        error: /ENOENT|NotFound|不存在|已移动|moved/i.test((error as Error).message)
          ? translate("ai.projectMovedOrInaccessible")
          : (error as Error).message,
      })
    }
  }, [scope, project.rootPath, setProject, patch, trusted])
  useEffect(
    () => () => {
      for (const controller of controllers.current.values()) controller.abort()
    },
    [],
  )
  useEffect(() => {
    if (scope !== "empty") void refresh()
  }, [scope, refresh])
  useEffect(() => {
    const listener = () => {
      void refresh()
    }
    window.addEventListener("envoi:ai-configured", listener)
    window.addEventListener("envoi:connection-updated", listener)
    return () => {
      window.removeEventListener("envoi:ai-configured", listener)
      window.removeEventListener("envoi:connection-updated", listener)
    }
  }, [refresh])
  const select = async (sessionId: string) => {
    if (current.busy || controllers.current.has(scope) || navigation.current.has(scope)) return
    navigation.current.add(scope)
    setNavigating(true)
    try {
      patch(scope, {
        record: await agentRequest<AgentRecord>("session", { projectId: scope, sessionId }),
        error: "",
      })
    } catch (error) {
      patch(scope, { error: (error as Error).message })
    } finally {
      navigation.current.delete(scope)
      setNavigating(false)
    }
  }
  const newSession = async () => {
    if (current.busy || controllers.current.has(scope) || navigation.current.has(scope)) return
    navigation.current.add(scope)
    setNavigating(true)
    try {
      const record = await agentRequest<AgentRecord>("new", { projectId: scope })
      patch(scope, (state) => ({
        ...state,
        record,
        sessions: [record, ...state.sessions],
        error: "",
      }))
    } catch (error) {
      patch(scope, { error: (error as Error).message })
    } finally {
      navigation.current.delete(scope)
      setNavigating(false)
    }
  }
  const send = async (message: string, context?: string) => {
    const id = scope
    if (!current.ready || current.busy || controllers.current.has(id) || navigation.current.has(id))
      return
    const controller = new AbortController()
    let sessionId = current.record?.id
    const writes = current.config?.tools === "write"
    if (writes && projectBusy) {
      patch(id, { error: translate("ai.waitProjectBusy") })
      return
    }
    if (writes && dirtyFiles(project).length) {
      try {
        if (!(await saveAll())) return
      } catch (error) {
        patch(id, { error: (error as Error).message })
        return
      }
    }
    controllers.current.set(id, controller)
    if (writes) setAgentBusy(id, true)
    patch(id, (state) => ({
      ...state,
      busy: true,
      root: project.rootPath,
      name: project.name,
      error: "",
      record: {
        ...(state.record ?? {
          id: "pending",
          name: translate("common.newSession"),
          status: "running",
        }),
        messages: [
          ...(state.record?.messages ?? []),
          { id: crypto.randomUUID(), role: "user", text: message },
          { id: crypto.randomUUID(), role: "assistant", text: "" },
        ],
      },
    }))
    try {
      for await (const event of agentChat(message, {
        projectId: id,
        sessionId,
        context,
        dirty: false,
        signal: controller.signal,
      })) {
        if (event.type === "session") {
          sessionId = event.id
          patch(id, (state) => ({
            ...state,
            record: state.record ? { ...state.record, id: event.id } : null,
          }))
        }
        if (
          event.type === "delta" ||
          event.type === "thinking" ||
          event.type === "tool" ||
          event.type === "metrics"
        )
          patch(id, (state) => ({
            ...state,
            record: state.record
              ? {
                  ...state.record,
                  messages: state.record.messages.map((item, index) =>
                    index === state.record!.messages.length - 1
                      ? appendChatEvent(item, event)
                      : item,
                  ),
                }
              : null,
          }))
        if (event.type === "error") throw Error(event.message)
      }
    } catch (error) {
      patch(id, {
        error: controller.signal.aborted ? translate("ai.taskStopped") : (error as Error).message,
      })
    } finally {
      navigation.current.add(id)
      controllers.current.delete(id)
      if (writes) setAgentBusy(id, false)
      if (sessionId) {
        try {
          const record = await agentRequest<AgentRecord>("session", { projectId: id, sessionId })
          patch(id, (state) => ({
            ...state,
            record,
            sessions: [record, ...state.sessions.filter((item) => item.id !== record.id)],
          }))
        } catch {
          /* Stream error already shown. */
        }
      }
      if (writes && project.rootPath) {
        try {
          const fresh = await readProject(project.rootPath)
          setProject((current) =>
            current.id === project.id ? mergeDrafts(fresh, current) : current,
          )
        } catch (error) {
          patch(id, { error: translate("ai.reopenToRefresh") + (error as Error).message })
        }
      }
      navigation.current.delete(id)
      patch(id, { busy: false })
      window.dispatchEvent(new Event("envoi:workspaces-updated"))
    }
  }
  const stop = () => {
    controllers.current.get(scope)?.abort()
    if (current.record?.id)
      void agentRequest("abort", { projectId: scope, sessionId: current.record.id }).catch(() => {})
  }
  const remove = async (id: string) => {
    try {
      await agentRequest("history/delete", { projectId: scope, sessionId: id, confirm: true })
      patch(scope, (state) => ({
        ...state,
        record: state.record?.id === id ? null : state.record,
        sessions: state.sessions.filter((item) => item.id !== id),
      }))
    } catch (error) {
      patch(scope, { error: (error as Error).message })
    }
  }
  return (
    <AgentContext.Provider
      value={{
        status,
        scope,
        record: current.record,
        sessions: current.sessions,
        busy: current.busy,
        navigating,
        error: current.error,
        ready: current.ready,
        config: current.config,
        refresh,
        select,
        newSession,
        send,
        stop,
        remove,
      }}
    >
      {Object.entries(scopes)
        .filter(([id, state]) => id !== scope && state.busy)
        .map(([id, state]) => (
          <div
            key={id}
            role="status"
            className="fixed bottom-8 right-3 z-50 flex max-w-lg gap-3 rounded border bg-card p-3 text-xs shadow-lg"
          >
            <span title={state.root}>后台任务：{state.name} · 仍在运行</span>
            <button onClick={() => controllers.current.get(id)?.abort()}>停止任务</button>
          </div>
        ))}
      {children}
    </AgentContext.Provider>
  )
}
