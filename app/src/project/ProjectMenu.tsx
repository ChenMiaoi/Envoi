...
export function ProjectMenu() {
  ...
  const browse = async (next: string[]) => {
    ...
    const children = listing.filter(entry=>entry.kind==='directory').map(entry=>entry.name).filter((part) => part && !part.startsWith(".") && part !== "node_modules");
    setTrail(next); setFolders(children.sort((a, b) => a.localeCompare(b)));
  };
  useEffect(()=>{let active=true;void authorizedRoots().then(async(items)=>{
  ...
  return <>
    ...
    <DropdownMenu><DropdownMenuTrigger aria-label={t('project.menuAria')} className="flex items-center gap-2 rounded focus-visible:outline focus-visible:outline-primary">
      ...
      <DropdownMenuItem disabled={busy || !project.rootPath} onSelect={() => openDialog("file")}>{t('project.newFile')}</DropdownMenuItem>
      ...
      <DropdownMenuSeparator /><DropdownMenuItem onSelect={() => openDialog("open")}>{t('project.recentMenu',{count:recent.length})}</DropdownMenuItem>
      ...
      <DialogContent className="gap-0 overflow-hidden p-0 max-h-[90vh] overflow-y-auto sm:max-w-[850px]">
        ...
        <div className="flex min-h-72">
          <aside className="w-44 shrink-0 border-r border-border bg-background/40 p-3">
            ...
            {roots.map((entry) => <button key={entry.id} disabled={busy || mode === "file" || !entry.path} className="mb-1 block w-full truncate rounded px-2 py-2 text-left text-xs hover:bg-secondary" onClick={() => void run(async () => { await rememberRoot(entry.path!); await browse([entry.path!]); })}>{entry.name}</button>)}
            {!project.rootPath && <p className="mb-3 px-2 text-[10px] text-muted-foreground">{t('project.notConnectedHint')}</p>}<div className="mb-3 flex items-center gap-2 px-2 text-xs text-muted-foreground"><HardDrive className="h-3.5 w-3.5" />{t('project.recentHeading')}</div>
            {!recent.length && <p className="px-2 text-xs text-muted-foreground">{t('project.noRecent')}</p>}
            {recent.map((entry) => <button key={entry.id} disabled={busy || mode === "file" || !entry.path} className="mb-1 flex w-full items-center gap-2 rounded px-2 py-2 text-left text-xs hover:bg-secondary disabled:opacity-40" onClick={() => void run(async () => {
          ...
          <div className="min-w-0 flex-1 space-y-4 p-5">
            {mode !== "file" ? <>
              <div className="flex items-center gap-2"><button disabled={busy || trail.length < 2} aria-label={t('project.parentDirAria')} className="rounded border border-border p-2 disabled:opacity-30" onClick={() => void run(() => browse(trail.slice(0,-1)))}><ArrowUp className="h-4 w-4" /></button><div className="min-w-0 flex-1 truncate rounded border border-border bg-background px-3 py-2 text-xs" title={location ?? ''}>{location ? location : t('project.noLocation')}</div></div>
              <div className="max-h-44 min-h-28 overflow-auto rounded-lg border border-border bg-background/40 p-2">
                {!location ? <div className="flex flex-col items-center gap-2 py-5 text-xs text-muted-foreground"><FolderOpen className="h-7 w-7" /><span>{t('project.emptyFolderHint')}</span></div> : !folders.length ? <p className="p-3 text-xs text-muted-foreground">{t('project.noSubfolders')}</p> : folders.map((folder) => <button key={folder} className="flex w-full items-center gap-2 rounded px-2 py-2 text-left text-xs hover:bg-secondary" disabled={busy} onClick={() => void run(() => browse([...trail, `${location}/${folder}`]))}><Folder className="h-4 w-4 text-primary" />{folder}</button>)}
              </div>
              <button disabled={busy} className="rounded border border-border px-3 py-2 text-xs hover:bg-secondary" onClick={() => void run(async () => {
                const root = await envoi().pickDirectory(); if (!root) return;
                await browse([root]); await rememberRoot(root); setRoots(await authorizedRoots());
        ...
        <div className="flex justify-end gap-2 border-t border-border bg-background/30 px-5 py-4"><button disabled={busy} onClick={() => setMode(null)} className="rounded border border-border px-4 py-2 text-xs">{t('project.cancel')}</button><button disabled={busy || (mode === "new" && enableGit && !gitAvailable) || (mode !== "file" && !location) || (mode !== "open" && !name.trim()) || (!!changed && mode !== "file" && !discard)} className="rounded bg-primary px-4 py-2 text-xs font-medium text-primary-foreground disabled:opacity-40" onClick={() => void run(async () => {
          if (mode === "file") {
