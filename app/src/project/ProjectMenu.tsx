...
export function ProjectMenu() {
  ...
  return <>
      ...
      <DialogContent className="gap-0 overflow-hidden p-0 max-h-[90vh] overflow-y-auto sm:max-w-[850px]">
        ...
        <div className="flex min-h-72">
          ...
          <div className="min-w-0 flex-1 space-y-4 p-5">
            ...
            {changed > 0 && mode !== "file" && <div className="rounded border border-warning/30 p-3 text-xs text-warning"><p>当前项目有 {changed} 个未保存文件。</p><button className="my-2 underline" disabled={busy} onClick={() => void saveAll()}>先保存全部</button><label className="flex items-center gap-2"><input type="checkbox" checked={discard} onChange={(event) => setDiscard(event.target.checked)} />切换时放弃未保存修改</label></div>}
          </div>
        </div>
    ...
    <Dialog open={connectionOpen} onOpenChange={setConnectionOpen}><DialogContent><DialogHeader><DialogTitle>项目本地连接</DialogTitle><DialogDescription>目录已获浏览器授权。旧项目首次连接本机工具时需补充绝对路径，之后 Git 等工具统一复用。</DialogDescription></DialogHeader><input aria-label="项目连接绝对路径" value={connectionPath} onChange={event=>setConnectionPath(event.target.value)} placeholder="当前项目目录的本地绝对路径" className="rounded border border-input bg-background px-3 py-2 text-xs" /><button disabled={busy||!connectionPath.startsWith('/')} className="rounded bg-primary px-3 py-2 text-xs text-primary-foreground disabled:opacity-40" onClick={()=>void run(async()=>{await bindProjectConnection(project.directory!,connectionPath);setConnectionOpen(false);setMessage('项目本地连接已保存，后续本机工具复用此连接。');})}>保存项目连接</button>{message.includes('身份冲突')&&<button disabled={busy||!connectionPath.startsWith('/')} className="text-left text-xs text-primary" onClick={()=>void run(async()=>{await bindAgentDirectory(project.directory!,connectionPath,true);await rememberGitPath(project.directory!,connectionPath);await activate(project.directory!);setConnectionOpen(false);})}>作为独立副本连接</button>}{message&&<p role="status" className="text-xs text-warning">{message}</p>}</DialogContent></Dialog>
