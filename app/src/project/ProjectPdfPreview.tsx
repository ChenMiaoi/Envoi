...
export function ProjectPdfPreview({target,syncPoint,onLocateSource}:{target?:{title:string;id:number};syncPoint?:{path:string;line:number;id:number};onLocateSource?:(path:string,line:number)=>void}) {
 ...
 useEffect(()=>{let live=true;void (async()=>{let bytes:Uint8Array|null=null;try{const file=project.files.find(f=>f.path==='build/main.synctex.gz')?.file;if(file)bytes=new Uint8Array(await file.arrayBuffer());}catch{/* 缺失或不可读时退化为无映射。 */}if(live)setDiskSync(bytes);})();return()=>{live=false;};},[project]);
