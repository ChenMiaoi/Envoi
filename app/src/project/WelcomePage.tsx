...
export function WelcomePage() {
  ...
  useEffect(() => {
    ...
    return () => {
      live = false
      window.removeEventListener("envoi:recent-updated", refresh)
    }
  }, [setMessage])
    ...
    <main data-testid="welcome-page" className="welcome-page h-full overflow-auto bg-background">
      <div className="welcome-panel mx-auto flex w-full max-w-2xl flex-col px-6 py-10 sm:px-10">
        ...
        <div className="flex flex-col gap-8">
          <section aria-labelledby="welcome-start" className={hasRecent ? "order-2" : undefined}>
            <h2
              id="welcome-start"
            ...
            <div className={hasRecent ? "flex flex-wrap gap-2" : "space-y-2"}>
              {actions.map((action) => (
                <button
                  key={action.event}
                  ...
                  className={`group flex items-center rounded-lg border border-border text-left transition-colors hover:border-primary/40 hover:bg-primary/5 focus-visible:outline focus-visible:outline-primary disabled:opacity-40 ${hasRecent ? "gap-2 px-3 py-2" : "w-full gap-4 bg-card p-4"}`}
                >
          ...
          {(loading || hasRecent) && (
            <section aria-labelledby="welcome-recent" className="order-1">
              ...
              {loading ? (
                <p className="py-5 text-sm text-muted-foreground">{t("project.processing")}</p>
              ) : recent.length ? (
                <div className="max-h-80 overflow-y-auto">
                  {recent.map((entry) => (
                    <div
                      key={entry.id}
                      className="group flex items-center rounded-lg hover:bg-secondary"
                    >
                      <button
                        disabled={blocked || !entry.path}
                        onClick={() =>
                          window.dispatchEvent(
                            new CustomEvent("envoi:open-recent", { detail: entry.path }),
                          )
                        }
                        className="flex min-w-0 flex-1 items-center gap-3 px-2 py-3 text-left focus-visible:outline focus-visible:outline-primary disabled:opacity-40"
                      >
                      ...
                      <button
                        disabled={blocked}
                        aria-label={t("welcome.remove", { name: entry.name })}
                        title={t("project.removeRecord")}
                        onClick={async () => {
                        ...
                        className="m-1 rounded p-2 text-muted-foreground hover:bg-background hover:text-foreground focus-visible:outline focus-visible:outline-primary disabled:opacity-40"
                      >
