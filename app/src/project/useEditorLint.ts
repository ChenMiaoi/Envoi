...
export function useEditorLint(fileId:string|undefined,path:string|undefined,text:string){
 ...
 useEffect(()=>{
  ...
  const timer=setTimeout(()=>void(async()=>{
   ...
   try{const runtimeResponse=await fetch('/api/envoi/compiler',{signal:controller.signal});if(!runtimeResponse.headers.get('content-type')?.includes('application/json'))throw Error('本地检查服务未连接');const runtime=await runtimeResponse.json();
