...
export function ChatPanel({ compact = false, inputOnly = false, context }: { compact?: boolean; inputOnly?: boolean; context?: ChatContext }) {
  ...
  function send() {
    ...
    setAttempted(true);
    setInput('');
    void agent.send(text, context ? t('chat.fileContext',{label:context.label,text:context.text}) : undefined);
  }
          ...
          <div className="flex items-end gap-2">
            ...
            <div className="shrink-0 pb-0.5 text-muted-foreground">
              {agent.busy
                ...
                : <button type="button" onClick={send} disabled={!available || !input.trim()} aria-label={t('chat.send')} className="rounded-full bg-foreground p-2 text-primary-foreground transition-opacity hover:opacity-85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-40"><ArrowUp className="h-4 w-4" /></button>}
            </div>
          </div>
