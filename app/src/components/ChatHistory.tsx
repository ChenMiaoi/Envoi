...
export function ChatHistory(){
 ...
 <p className="mb-2 text-xs text-muted-foreground">{t('chat.historyScope')}</p><input aria-label={t('chat.searchAria')} placeholder={t('chat.searchPlaceholder')} value={query} onChange={event=>{setLoading(true);setQuery(event.target.value);}} className="mb-2 w-full rounded border bg-background px-2 py-1.5 text-sm outline-none"/>
