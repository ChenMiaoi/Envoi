...
export function ReferencesPanel({ paper, editor, onLocate }: { paper: ReturnType<typeof collectPaper>; editor: React.RefObject<LatexEditorHandle | null>; onLocate: (location: SourceLocation) => void }) {
  ...
  const result = useMemo(() => {
    try {
      ...
      if (local) return { entries: parseBibliography(local.text), label: t('refs.localFileLabel',{name:local.name}), error: "" };
      ...
      const resolved = declarations.map(({ name, from }) => {
        ...
        if (!match || match.text === undefined) throw new Error(t('refs.bibUnreadable',{from,name}));
        return match;
      });
      const unique = [...new Map(resolved.map((file) => [file.id, file])).values()];
      return { entries: parseBibliography(unique.map((file) => file.text).join("\n")), label: t('refs.projectFilesLabel',{files:unique.map((file) => file.path).join("、")}), error: "" };
    } catch (error) { return { entries: [], label: local?.name || t('refs.projectBib'), error: (error as Error).message }; }
  ...
  const locate = (key: string) => {
    ...
    setNotice(t('refs.citationLocation',{key,file:positions[index].path,index:index+1,total:positions.length}));
  };
  return <div className="space-y-2 text-[11px]">
    <div className="flex flex-wrap gap-2">
      <button className="rounded border border-border px-2 py-1 hover:bg-secondary" onClick={() => fileRef.current?.click()}>{t('refs.openBib')}</button>
      {local && <button className="text-muted-foreground" onClick={() => { request.current++; setLocal(null); setReadError(""); }}>{t('refs.useProjectBib')}</button>}
      <input ref={fileRef} type="file" accept=".bib,text/plain" aria-label={t('refs.selectLocalBib')} className="hidden" onChange={async (event) => {
        ...
        catch (error) { if (id === request.current) setReadError(t('refs.readFailed',{message:(error as Error).message})); }
      }} />
    </div>
    <p className="break-words text-muted-foreground">{result.label}</p>
    <p className="text-muted-foreground">{t('refs.scopeHint')}</p>
    {!!paper.missing.length && <p role="alert" className="text-warning">{t('refs.statsIncomplete',{list:paper.missing.join("；")})}</p>}
    {result.error && <p role="alert" className="break-words text-danger">{t('refs.parseFailed',{error:result.error})}</p>}
    ...
    {[true, false].map((used) => {
      ...
      return <section key={String(used)} className="space-y-1.5">
        <h3 className="pt-2 font-medium text-muted-foreground">{used ? t('refs.cited') : t('refs.uncited')} · {entries.length}</h3>
        {entries.map((entry) => {
          ...
          return <div key={entry.key} className="rounded-lg border border-border bg-background p-2.5">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <button className="rounded bg-primary px-2 py-1 text-primary-foreground" onClick={() => { editor.current?.insert(`\\cite{${entry.key}}`); setNotice(t('refs.inserted',{key:entry.key})); }}>{t('refs.insertCitation')}</button>
              {!!count && <button className="rounded border border-border px-2 py-1 hover:bg-secondary" onClick={() => locate(entry.key)}>{t('refs.locateCitations',{count})}</button>}
            </div>
          </div>;
        })}
      </section>;
    })}
    {!!unknown.length && <section className="space-y-1 border-t border-border pt-2 text-warning"><h3>{t('refs.unresolvedHeading')}</h3>{unknown.map((key) => <button key={key} onClick={() => locate(key)} className="block break-all text-left">{t('refs.locateKey',{key})}</button>)}</section>}
  </div>;
}
