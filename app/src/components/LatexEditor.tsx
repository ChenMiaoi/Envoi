...
export function LatexEditor({ref,value,onChange,fontSize=12.5,highlight,fontFamily,lineHeight=1.75,tabSize=4,onDoubleClickLine}: {
  ...
  const lines=useMemo(()=>value.split("\n").map(text=>({text,tokens:tokenizeLatex(text)})),[value]);
  ...
  const reveal=useCallback((start:number,end:number)=>{
    const textarea=textareaRef.current;if(!textarea)return;
    ...
    Object.assign(mirror.style,{position:"fixed",top:"0",left:"0",visibility:"hidden",width:`${textarea.clientWidth}px`,boxSizing:"border-box",whiteSpace:"pre-wrap",overflowWrap:"anywhere",wordBreak:"normal",font:style.font,lineHeight:style.lineHeight,letterSpacing:style.letterSpacing,tabSize:style.tabSize,padding:style.padding,border:"0"});
    mirror.append(document.createTextNode(textarea.value.slice(0,start)));
    const marker=document.createElement("span");marker.textContent=textarea.value.slice(start,start+1)||"\u200b";mirror.append(marker,document.createTextNode(textarea.value.slice(start+1)));
  ...
  useImperativeHandle(ref,()=>({insert(text){const textarea=textareaRef.current;const result=replaceSelection(value,textarea?.selectionStart??value.length,textarea?.selectionEnd??value.length,text);pendingCursor.current=result.cursor;onChange(result.value);},locate:reveal}));
  useLayoutEffect(()=>{
    const textarea=textareaRef.current,content=contentRef.current,gutter=gutterRef.current;if(!textarea||!content||!gutter)return;
    const measure=()=>{
      ...
      sourceLines.forEach((line,index)=>{const number=gutter.children[index] as HTMLElement;number.style.height=`${line.getBoundingClientRect().height}px`;});
      const selected=highlight?value.slice(0,highlight.start).split('\n').length-1:-1;
      const target=sourceLines[selected],band=highlightRef.current;
      if(band&&target){band.style.top=`${target.offsetTop}px`;band.style.height=`${target.getBoundingClientRect().height}px`;}
      sync();
    };
    measure();const observer=new ResizeObserver(measure);observer.observe(textarea);observer.observe(content);
    let active=true;void document.fonts.ready.then(()=>{if(active)measure();});
    return()=>{active=false;observer.disconnect();};
  },[value,fontSize,fontFamily,lineHeight,tabSize,highlight,sync]);
  useLayoutEffect(()=>{if(pendingCursor.current!==null){reveal(pendingCursor.current,pendingCursor.current);pendingCursor.current=null;}},[value,reveal]);
  return <div data-content-typography="editor" className="flex h-full min-w-0 bg-editor">
    <div ref={gutterRef} aria-hidden className="select-none shrink-0 overflow-hidden border-r border-border/60 py-3 pl-3 pr-2.5 text-right font-editor text-muted-foreground/40" style={typography}>
      {lines.map((_,index)=><div key={index} style={{width:`${String(lines.length).length}ch`}}>{index+1}</div>)}
    </div>
