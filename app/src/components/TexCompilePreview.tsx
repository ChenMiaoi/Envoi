...
function createLinkService(
  ...
  pagesRef: React.RefObject<PDFPageProxy[]>,
  containerRef: React.RefObject<HTMLDivElement | null>,
) {
  const goToPage = (pageNumber: number) => {
    const total = pagesRef.current.length
    if (total)
      scrollToPagePosition(containerRef.current, Math.min(Math.max(1, pageNumber), total), 0)
  }
  const service = {
    ...
    async goToDestination(dest: string | unknown[]) {
      ...
      const index =
        typeof explicit[0] === "number" ? explicit[0] : await doc.getPageIndex(explicit[0] as never)
      ...
      if (mode === "XYZ" && typeof explicit[3] === "number") {
        const [, y] = viewport.convertToViewportPoint(
          (explicit[2] as number) ?? 0,
          explicit[3] as number,
        )
    ...
    executeNamedAction(action: string) {
      ...
      for (const figure of figures)
        if (figure.getBoundingClientRect().top <= top + 40) current = Number(figure.dataset.pdfPage)
...
function PdfPage({
  ...
  onClickPoint?: (page: number, x: number, y: number) => void
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => setVisible(entry.isIntersecting), {
      root: figure.parentElement?.parentElement,
  ...
  useEffect(() => {
      ...
      try {
        ...
        task.onContinue = (resume: () => void) => {
          void scheduler
            .wait(signal)
            .then(() => {
              if (active && !signal.aborted) resume()
            })
            .catch(() => {})
        }
        await task.promise
        ...
        if (annotation) {
          ...
          if (active && !signal.aborted) {
            ...
            await layer.render({
              ...
              div: annotation,
              page,
              linkService: linkService as PDFLinkService,
              annotations,
              renderForms: false,
            })
          }
        ...
        if (active && !signal.aborted && failure.name !== "RenderingCancelledException") {
          const message = t("compile.renderPageFailed", {
    ...
    return () => {
      ...
      canvas.height = 0
      text.innerHTML = ""
      if (annotationElement) annotationElement.innerHTML = ""
    }
  ...
  return (
    <figure
      ...
      aria-label={t("compile.page", { page: page.pageNumber })}
    >
      <div
      ...
      >
        <canvas
          ...
          aria-label={t("compile.pdfPage", { page: page.pageNumber })}
        />
        {separator && (
          <span
            aria-hidden="true"
            className="pointer-events-none absolute inset-x-0 top-0 h-px bg-slate-300/50"
          />
        )}
        {highlight !== undefined && (
          <div
            className="pointer-events-none absolute left-0 right-0 h-6 border-t-2 border-amber-400/60 bg-amber-300/15"
            style={{ top: (highlight * width) / original.width }}
          />
        )}
        <div ref={textRef} className="textLayer" />
        <div ref={annotationRef} className="annotationLayer" />
        {error && (
          <p role="alert" className="absolute inset-3 text-sm text-red-600">
            {error}
          </p>
        )}
      </div>
...
export function TexCompilePreview({
  initialSource,
  paperOnly = false,
  target,
  syncReady,
  ...
  reading?: { page: number; fraction: number }
  onReadingChange?: (position: { page: number; fraction: number }) => void
  target?: { title: string; id: number }
  syncReady?: boolean | null
  paperOnly?: boolean
  initialSource?: { file?: File; name: string; url?: string }
  syncData?: Uint8Array | null
  sourcePaths?: string[]
  syncPoint?: { path: string; line: number; id: number }
  onLocateSource?: (path: string, line: number) => void
}) {
  const [source, setSource] = useState<{ file?: File; name: string; url?: string }>(
    initialSource ?? { name: "" },
  )
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      scroller.removeEventListener("scroll", pause)
      scheduler.dispose()
    }
  }, [scheduler])
  useEffect(() => {
    if (jump && pages.length) scrollToPagePosition(containerRef.current, jump.page, 0)
  }, [jump, pages])
  const initialReading = useRef(reading),
    restoredReading = useRef(false),
    readingCallback = useRef(onReadingChange)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const save = () => {
      ...
      const top = scroller.getBoundingClientRect().top
      const figures = Array.from(container.querySelectorAll<HTMLElement>("[data-pdf-page]"))
      const figure = figures.find((el) => el.getBoundingClientRect().bottom > top + 5)
      if (figure) {
        lastPosition = {
  ...
  useEffect(() => {
    setLinkService(createLinkService(documentRef, pagesRef, containerRef))
  }, [])
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        if (syncData) db = parseSyncTex(await gunzipSyncTex(syncData))
      } catch {
        /* 映射损坏时退化为仅渲染。 */
      }
  ...
  const pathOfInput = useCallback(
    (input: number) =>
      syncDb && sourcePaths ? matchSyncTexPath(sourcePaths, syncDb.inputs[input]) : undefined,
    [syncDb, sourcePaths],
  )
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(() => {
      ...
      if (!hit) {
        setSyncNotice(t("compile.noSyncPosition"))
        return
      }
      ...
      setHighlight({ page: hit.page, y: hit.y })
      setSyncNotice("")
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setPages(loaded)
        setStatus(t("compile.loadedPages", { count: document.numPages }))
        setLogs((previous) => [
          ...previous,
        ...
        const message =
          failure.name === "PasswordException"
            ? t("compile.passwordPdf")
            : t("compile.loadFailed", { message: failure.message })
        setError(message)
        setStatus(t("compile.loadError"))
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!bookmark?.dest) {
        if (active) setSyncNotice(t("compile.noBookmark"))
        return
      }
      const destination =
        typeof bookmark.dest === "string" ? await pdf.getDestination(bookmark.dest) : bookmark.dest
      if (!destination || destination[1]?.name !== "XYZ") {
        if (active) setSyncNotice(t("compile.bookmarkFormatUnsupported"))
        return
      }
      const index =
      ...
      const [, y] = viewport.convertToViewportPoint(
        destination[2] ?? 0,
        destination[3] ?? viewport.viewBox[3],
      )
      const figure = containerRef.current?.querySelector<HTMLElement>(
      ...
      scroller.scrollTo({
        top:
          ...
          figure.getBoundingClientRect().top -
          scroller.getBoundingClientRect().top +
          (y * width) / viewport.width -
          20,
        behavior: "smooth",
      })
      setHighlight({ page: index + 1, y })
      setSyncNotice("")
      timer = setTimeout(() => setHighlight(null), 1800)
    })().catch(() => {
      if (active) setSyncNotice(t("compile.bookmarkReadFailed"))
    })
    return () => {
      active = false
      if (timer) clearTimeout(timer)
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-editor">
      {!paperOnly && (
        <div className="flex min-h-9 shrink-0 flex-wrap items-center justify-between gap-2 border-b border-border px-3 py-1 text-[11.5px]">
          <div className="flex min-w-0 items-center gap-2">
            <span className="max-w-40 truncate" title={source.name}>
              {source.name}
            </span>
            <button
              type="button"
              ...
              className="flex items-center gap-1 rounded px-1 py-0.5 text-muted-foreground hover:bg-secondary"
            >
              {status}
              <ChevronDown className={`h-3 w-3 ${logOpen ? "rotate-180" : ""}`} />
            </button>
          </div>
          <button
            type="button"
      ...
      {paperOnly && navigationNotice && (
        <p role="status" className="shrink-0 px-3 py-1 text-[10px] text-muted-foreground">
          {navigationNotice}
        </p>
      )}
        ...
        <div data-content-typography="pdf" ref={containerRef} className="flex w-full flex-col">
          ...
          {pages.map((page, index) => (
            <PdfPage
              ...
              onClickPoint={syncDb && onLocateSource ? clickPoint : undefined}
              highlight={highlight?.page === page.pageNumber ? highlight.y : undefined}
            />
          ))}
        </div>
      </div>
