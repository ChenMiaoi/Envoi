...
export function PaperOutline({nodes,warnings,onLocate}:{nodes:OutlineNode[];warnings:string[];onLocate:(location:SourceLocation,title:string)=>void}) {
 ...
 return <nav aria-label="论文自动目录">
  ...
  {!!warnings.length&&<details className="mt-3 px-2 text-[10px] text-warning"><summary>有 {warnings.length} 个结构提示</summary>{warnings.map(w=><p key={w} className="mt-1 break-words">{w}</p>)}</details>}
 </nav>;
}
