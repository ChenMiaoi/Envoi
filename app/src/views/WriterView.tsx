...
export function WriterView({
  ...
  requestedFile?: { id: string; request: number }
  problemTarget?: import("@/project/ProblemsPanel").ProblemTarget
}) {
  const location = useLocation()
  const { effective } = useSettings()
  const [pdfTarget, setPdfTarget] = useState<{ title: string; id: number } | undefined>()
  const { t } = useT()
  const [syncPoint, setSyncPoint] = useState<
    { path: string; line: number; id: number } | undefined
  >()
  ...
  const sources = useMemo(
    () =>
      project.files
        ...
        .map((file) => ({ id: file.id, path: file.path, text: file.text! })),
    [project.files],
  )
  const [activeId, setActiveId] = useState(
    requestedFile?.id ?? problemTarget?.fileId ?? project.rootId,
  )
  const [lastRequested, setLastRequested] = useState(requestedFile)
  if (requestedFile && requestedFile !== lastRequested) {
    setLastRequested(requestedFile)
  ...
  useEffect(() => {
    const save = (event: Event) => {
      if (
        ...
        window.dispatchEvent(new Event("envoi:save-compile"))
      }
    }
  ...
  useLayoutEffect(() => {
    if (pendingLocation.current) {
      ...
      editor.current?.locate(location.start, location.end)
    }
  }, [activeId])

  const [lastProblem, setLastProblem] = useState(problemTarget)
  if (problemTarget && problemTarget !== lastProblem) {
    setLastProblem(problemTarget)
    setActiveId(problemTarget.fileId)
  }
  useLayoutEffect(() => {
    if (problemTarget?.fileId === activeId)
      editor.current?.locate(problemTarget.start, problemTarget.end)
  }, [problemTarget, activeId])
          ...
          <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-2">
            {tab === "outline" && (
              <PaperOutline
                ...
                onLocate={(location, title) => {
                  locate(location)
                  setPdfTarget((current) => ({ title, id: (current?.id ?? 0) + 1 }))
                }}
              />
            )}
            <div hidden={tab !== "refs"}>
              <ReferencesPanel paper={paper} editor={editor} onLocate={locate} />
            </div>
            <div hidden={tab !== "assets"}>
              <AssetsPanel paper={paper} onLocate={locate} />
            </div>
          </div>
        </div>
      </Panel>
            ...
            <div className="flex h-full flex-col">
              <div className="flex h-9 shrink-0 items-center justify-between border-b border-border bg-card px-3">
                <div className="flex items-center gap-2 text-[12px]">
                  <select
                    aria-label={t("writer.currentFileAria")}
                    className="max-w-48 bg-card text-foreground/90"
                    value={activeId}
                ...
                <span className="font-editor text-[11px] text-muted-foreground">
                  {project.files.some((file) => file.text !== file.saved)
                    ? t("writer.unsavedHint")
                    : t("writer.savedHint")}
                </span>
              </div>
              <div className="min-h-0 flex-1">
                {active ? (
                  <LatexEditor
                    ...
                    highlight={
                      problemTarget?.fileId === activeId
                        ? { start: problemTarget.start, severity: problemTarget.severity }
                        : undefined
                    }
                    onDoubleClickLine={(line) =>
                      setSyncPoint((current) => ({
                        path: active.path,
                        line,
                        id: (current?.id ?? 0) + 1,
                      }))
                  ...
                  <p className="p-4 text-sm text-muted-foreground">{t("writer.noLatex")}</p>
                )}
              </div>
            </div>
          </div>
