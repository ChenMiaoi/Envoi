...
const ai=await get('agent'),data=await get('data');const {getProviders}=await import('@earendil-works/pi-ai/compat');assert(getProviders().every(id=>ai.providers.some(provider=>provider.id===id)));assert.equal(new Set(ai.providers.map(provider=>provider.id)).size,ai.providers.length);
