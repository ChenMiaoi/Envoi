import {mkdtemp,mkdir,writeFile,rm} from 'node:fs/promises';import {tmpdir} from 'node:os';import path from 'node:path';import {execFileSync} from 'node:child_process';import assert from 'node:assert/strict';
import {initializeBoundGit,readBoundGitLog,readBoundGitShow,parseLog,parseRefs} from '../server/git.mjs';
...
try{
 ...
 await writeFile(path.join(root,'main.tex'),'one\n');git('add','main.tex');git('commit','-m','Initial draft');
 git('checkout','-b','feature');await writeFile(path.join(root,'feature.tex'),'two\n');git('add','feature.tex');git('commit','-m','Feature section');
 git('checkout','main');await writeFile(path.join(root,'main.tex'),'one\nmain edit\n');git('add','main.tex');git('commit','-m','Main edit');
 git('merge','--no-ff','feature','-m','Merge feature');git('tag','v0.1');
 const log=await readBoundGitLog(input);
 assert.equal(log.state,'ready');assert.equal(log.branch,'main');assert.equal(log.commits.length,4);assert.equal(log.truncated,false);
 const bySubject=Object.fromEntries(log.commits.map(c=>[c.subject,c])),merge=bySubject['Merge feature'],mainEdit=bySubject['Main edit'],feature=bySubject['Feature section'],initial=bySubject['Initial draft'];
 assert.equal(merge.parents.length,2);assert.equal(merge.parents[0],mainEdit.hash);assert.equal(merge.parents[1],feature.hash);
 assert(feature.refs.some(r=>r.name==='feature'));assert.equal(initial.parents.length,0);
 const shown=await readBoundGitShow({...input,commit:merge.hash});
 assert.equal(shown.commit.subject,'Merge feature');assert.deepEqual(shown.files.map(f=>[f.path,f.added]),[['feature.tex',1]]); // merge described against first parent
