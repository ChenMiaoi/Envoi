...
try {
  ...
  await app.evaluate(({ ipcMain, dialog }) => {
    ...
    globalThis.catalogFixture = { calls: 0, models: [], fail: false, checkedAt: Date.now() }
      ...
      providers: [
        {
          ...
          catalog: {
            state: globalThis.catalogFixture.fail ? "failed" : "cache",
            source: "fixture",
            checkedAt: globalThis.catalogFixture.checkedAt,
          },
        },
      ],
      models: globalThis.catalogFixture.models,
      settings,
      storage: {},
    }))
    ipcMain.removeHandler("envoi:agent-request")
    ipcMain.handle("envoi:agent-request", async (_event, route) => {
      if (route === "sessions") return { sessions: [], settings }
      if (route === "models/refresh") {
        globalThis.catalogFixture.calls++
        await new Promise((resolve) => setTimeout(resolve, 600))
        if (globalThis.catalogFixture.fail)
          return { ok: false, error: "Fixture directory unavailable" }
        globalThis.catalogFixture.checkedAt = Date.now()
        globalThis.catalogFixture.models = [
          {
            id: "fixture-model",
            provider: "opencode-go",
            name: "Fixture discovered model",
            available: true,
            thinkingLevels: [],
          },
        ]
  ...
  assert.equal(await app.evaluate(() => globalThis.catalogFixture.calls), 1)
  await page.keyboard.press("Escape")
  await menu.click()
  await page.waitForTimeout(800)
  assert.equal(
    await app.evaluate(() => globalThis.catalogFixture.calls),
    1,
    "populated catalog is not refetched",
  )
  await page.keyboard.press("Escape")
  await app.evaluate(() => {
    globalThis.catalogFixture.checkedAt = Date.now()
    globalThis.catalogFixture.models = []
    globalThis.catalogFixture.fail = true
  })
  ...
  assert.equal(
    await app.evaluate(() => globalThis.catalogFixture.calls),
    2,
    "failed discovery does not loop",
  )
  await page.keyboard.press("Escape")
  await app.evaluate(() => {
    globalThis.catalogFixture.fail = false
  })
  await menu.click()
  await page.getByRole("button", { name: "Fixture discovered model", exact: true }).waitFor()
  assert.equal(
    await app.evaluate(() => globalThis.catalogFixture.calls),
    3,
    "reopening retries an empty catalog",
  )
  await page.getByRole("button", { name: "刷新模型目录", exact: true }).click()
  await page.waitForTimeout(900)
  assert.equal(await app.evaluate(() => globalThis.catalogFixture.calls), 4)
  assert.equal(await page.getByText(/尚无模型发现适配/).count(), 0)
  await page.keyboard.press("Escape")
  await app.evaluate(() => {
    globalThis.catalogFixture.checkedAt = Date.now() - 300001
  })
  await page.evaluate(() => window.dispatchEvent(new Event("envoi:ai-configured")))
  await page.waitForTimeout(300)
  await menu.click()
  ...
  assert.equal(
    await app.evaluate(() => globalThis.catalogFixture.calls),
    5,
    "stale populated catalog refreshes automatically",
  )
  console.log(
    "PASS model menu: automatic discovery, loading, cached options, failure and retry without request loop",
  )
} finally {
