...
await build({
  entryPoints: ["src/lib/gitGraph.ts"],
  bundle: true,
  platform: "node",
  format: "esm",
  outfile: "tmp/compile-check/gitGraph.mjs",
})
const { layoutGraph } = await import("../tmp/compile-check/gitGraph.mjs")
const mk = (hash, parents = [], extra = {}) => ({
  hash,
  parents,
  refs: [],
  head: false,
...
const { rows, edges, laneCount } = layoutGraph([
  mk("M", ["c", "b"]),
  mk("c", ["a"]),
  mk("b", ["a"]),
  mk("a"),
...
assert.deepEqual(
  rows.map((r) => [r.commit.hash, r.lane]),
  [
    ["M", 0],
    ["c", 0],
    ["b", 1],
...
const edge = (from, to) =>
  edges.find((e) => rows[e.fromRow].commit.hash === from && rows[e.toRow]?.commit.hash === to)
