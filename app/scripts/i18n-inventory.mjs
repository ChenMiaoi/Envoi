...
function walk(dir){const out=[];for(const name of readdirSync(dir)){const p=join(dir,name);if(statSync(p).isDirectory())out.push(...walk(p));else if(/\.(tsx|ts)$/.test(name))out.push(p);}return out;}

const files=walk(src);
const cjk=/[\u3400-\u9fff\uf900-\ufaff]/;
const literal=/(['"`])((?:\\.|(?!\1)[\s\S])*?)\1/g;
const attr=/\b(?:aria-label|title|placeholder|alt|aria-description)=(["'])((?:\\.|(?!\1)[\s\S])*?)\1/g;
...
function dump(map){return [...map.entries()].map(([value,locs])=>{
 const first=locs[0];
