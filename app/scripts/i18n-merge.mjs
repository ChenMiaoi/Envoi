...
if(!existsSync(fragmentsDir)){console.log('no fragments directory');process.exit(0);}

const files=readdirSync(fragmentsDir).filter(f=>f.endsWith('.json')).sort();
if(!files.length){console.log('no fragments');process.exit(0);}

const merged={}; // key -> {zh,en,ja,source}
for(const file of files){
 const data=JSON.parse(readFileSync(join(fragmentsDir,file),'utf8'));
 for(const [key,value] of Object.entries(data)){
  if(!/^[a-z][a-zA-Z]*\.[a-zA-Z][a-zA-Z0-9.]*$/.test(key))throw new Error(`${file}: bad key ${key}`);
  if(merged[key])throw new Error(`duplicate key across fragments: ${key} (${file} vs ${merged[key].source})`);
  for(const lang of ['zh','en','ja'])if(typeof value?.[lang]!=='string'||!value[lang])throw new Error(`${file}: ${key} missing ${lang}`);
...
for(const [lang,file] of Object.entries(targets)){
 ...
 for(const key of Object.keys(merged))if(have.has(key))throw new Error(`${file}: key already exists: ${key}`);
}
