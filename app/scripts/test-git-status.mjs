...
assert.deepEqual(
  parseGitStatus("R  renamed file.tex\0old file.tex\0?? new.tex\0").map((f) => [
    f.path,
    f.originalPath,
    f.untracked,
  ]),
  [
    ["renamed file.tex", "old file.tex", false],
    ["new.tex", undefined, true],
  ],
)
const root = await mkdtemp(path.join(tmpdir(), "envoi-git-status-")),
  proof = "b".repeat(64),
  input = { directory: root, proof }
try {
  await mkdir(path.join(root, ".envoi"))
  await writeFile(path.join(root, ".envoi/git-proof"), proof)
  assert.equal((await readBoundGitStatus(input)).state, "not-initialized")
  ...
  assert(result.files.some((f) => f.index === "A" && f.worktree === "M"))
  ...
  console.log(
    "PASS native Git status: uninitialized/clean/untracked/staged+modified; index unchanged, no remote",
  )
} finally {
