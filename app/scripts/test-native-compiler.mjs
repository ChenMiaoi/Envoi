...
if(!runtime.available){console.log('SKIP native compiler:',runtime.error);process.exit(0);}
...
try {
 ...
 await writeFile(path.join(root,'main.tex'),'\\documentclass{article}\\begin{document}\\input{chapters/body}\\end{document}');
 for(const engine of ['pdflatex','xelatex']){
  const result=await compileSnapshot({engine,main:'main.tex',drafts:[]},{trustedRoot:root,sourceRoot:root});
