    ...
    try {
      ...
      for (const change of changes) {
        if (
          ...
          !(typeof change.expectedText === "string" || change.expectedText === null)
        )
          throw Error("缺少文件保存基线")
        const target = await destination(root, change.path)
        if (seen.has(target)) throw Error("同一文件不能重复保存")
        seen.add(target)
        const expected = change.expectedText === null ? null : Buffer.from(change.expectedText)
        if (!equal(await contents(target), expected))
          throw Error(`${change.path} 已被外部修改，草稿已保留。`)
      ...
      for (const item of prepared)
        item.temporary = await prepare(item.target, Buffer.from(item.text))
      for (const item of prepared) {
        if (!equal(await contents(item.target), item.expected))
          throw Error(`${item.path} 在保存期间被外部修改，草稿已保留。`)
      ...
      await Promise.all(
        prepared
          ...
          .map((item) => rm(item.temporary, { force: true })),
      )
    }
  })
}
