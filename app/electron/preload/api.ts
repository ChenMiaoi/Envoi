...
export interface EnvoiBridge {
  ...
  compile(input: { rootPath: string; main: string; engine: string; files?: { path: string; base64: string }[]; drafts?: {path: string; text: string}[] }): Promise<{ ok: boolean; pdf?: string; synctex?: string | null; log: string; error?: string }>
  cancelCompile(): Promise<void>
  lint(input: { rootPath?: string; path: string; text: string; disabledRules?: number[] }): Promise<{ available: boolean; items?: unknown[]; error?: string }>
  ...
  dataPut(store: string, value: unknown, key?: string, opts?: { migrate?: boolean; expectedRevision?: number }): Promise<unknown>
  ...
  fsWriteFiles(root: string, files: { path: string; text?: string; base64?: string }[]): Promise<void>
