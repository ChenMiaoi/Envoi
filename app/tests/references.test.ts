...
test('Bib nested braces, string macros, author lists, malformed and duplicate keys', () => {
 const entries=parseBibliography('@string{venue="Journal"}\n@article{a,title={A {nested} title},author={Doe, Jane and Smith, John},journal=venue,year=2024}\n@book{b,title="Other"}');
 assert.equal(entries.length,2); assert.match(entries[0].title,/nested/);assert.match(entries[0].author,/Jane/);assert.equal(entries[0].venue,'Journal');
 assert.throws(()=>parseBibliography('@article{bad,title={unfinished'));assert.throws(()=>parseBibliography('@article{a,title={One}} @book{a,title={Two}}'));
...
\% visible \cite{d} \nocite{ignored}`;
 const found=findCitations(source);assert.deepEqual(found.map(x=>x.key),['a','b','a','b','a','c','d']);
 ...
 assert.equal(findCitations(String.raw`\cite{a}`).length,1);assert.equal(findCitations('').length,0);
});
test('reachable chapters only, cycles and missing files',()=>{
 const files=[{id:'main',path:'src/main.tex',text:String.raw`\input{method} % \input{unused}
\include{missing}`},{id:'method',path:'src/method.tex',text:String.raw`\cite{a}\input{main}`},{id:'unused',path:'src/unused.tex',text:String.raw`\cite{b}`}];
 const paper=collectPaper(files,'main');assert.deepEqual(paper.files.map(x=>x.id),['main','method']);assert.equal(paper.citations[0].key,'a');assert.equal(paper.citations[0].fileId,'method');assert.equal(paper.missing.length,1);
});
test('insertion replaces only selection, repeats add citations and caret advances',()=>{
 const first=replaceSelection('before SELECT after',7,13,String.raw`\cite{a}`);assert.equal(first.value,String.raw`before \cite{a} after`);
 const second=replaceSelection(first.value,first.cursor,first.cursor,String.raw`\cite{a}`);assert.equal(findCitations(second.value).length,2);assert.equal(second.value.slice(second.cursor),' after');
});
