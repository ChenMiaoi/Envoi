...
test('outside paths and unreliable positions do not become editor targets',()=>{
 ...
 assert.equal(items[0].path,undefined);assert.equal(diagnosticLocation(items[0],files),undefined);assert(!safeDiagnosticText('/private/tmp/paperdesk-tex-abc/project/main.tex').includes('/private'));
});
