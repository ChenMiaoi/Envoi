...
class MemoryFile {
 ...
 async getFile() { return new File([this.contents], this.name); }
 async createWritable() {
  if (this.failWrite) throw new DOMException('Permission denied', 'NotAllowedError');
  let buffer = this.contents;
  return { write: async (text: string) => { buffer = text; }, close: async () => { this.contents = buffer; }, abort: async () => {} };
 }
}
class MemoryDirectory {
 ...
 async getDirectoryHandle(name: string, options?: { create?: boolean }) {
  ...
  if (child && child.kind !== 'directory') throw new DOMException('File occupies name', 'TypeMismatchError');
  if (child) return child;
  if (!options?.create) throw new DOMException('Missing', 'NotFoundError');
  const result = new MemoryDirectory(name); this.children.set(name, result); return result;
 }
 async getFileHandle(name: string, options?: { create?: boolean }) {
  const child = this.children.get(name);
  if (child && child.kind !== 'file') throw new DOMException('Directory occupies name', 'TypeMismatchError');
  if (child) return child;
  if (!options?.create) throw new DOMException('Missing', 'NotFoundError');
...
test('new project creates a real directory model and never overwrites a namesake',async()=>{
 ...
 const tree=projectTree(project.files,project.directories); assert(tree.find(n=>n.name==='assets')?.kind==='folder');
});
test('saving persists edits, detects external changes before writing anything',async()=>{
 const root=new MemoryDirectory('paper'); root.children.set('main.tex',new MemoryFile('main.tex','original')); root.children.set('references.bib',new MemoryFile('references.bib','bib'));
 const project=await readProject(root.asHandle());
 project.files.find(f=>f.id==='main.tex')!.text='edited';
 const saved: string[]=[]; await saveProject(project,(id,text)=>{ saved.push(id); project.files.find(f=>f.id===id)!.saved=text; });
 assert.deepEqual(saved,['main.tex']); assert.equal((root.children.get('main.tex') as MemoryFile).contents,'edited'); assert.equal(dirtyFiles(project).length,0);
 project.files.find(f=>f.id==='main.tex')!.text='next'; project.files.find(f=>f.id==='references.bib')!.text='new bib';
 (root.children.get('references.bib') as MemoryFile).contents='external';
 await assert.rejects(saveProject(project,()=>assert.fail('must not save')),/外部修改/);
 assert.equal((root.children.get('main.tex') as MemoryFile).contents,'edited');assert.equal(dirtyFiles(project).length,2);
});
test('permission/write failure retains unsaved data and marks only completed writes',async()=>{
 const root=new MemoryDirectory('paper');root.children.set('a.tex',new MemoryFile('a.tex','a'));const blocked=new MemoryFile('b.tex','b');blocked.failWrite=true;root.children.set('b.tex',blocked);
 const project=await readProject(root.asHandle());project.files.forEach(f=>f.text+=' changed');const saved:string[]=[];
 await assert.rejects(saveProject(project,(id,text)=>{saved.push(id);project.files.find(f=>f.id===id)!.saved=text;}));
 assert.deepEqual(saved,['a.tex']);assert.equal(dirtyFiles(project)[0].id,'b.tex');assert.equal(blocked.contents,'b');
});
test('loading another project produces independent sources without mutating dirty project',async()=>{
 const first=new MemoryDirectory('first');first.children.set('main.tex',new MemoryFile('main.tex','first'));
 const second=new MemoryDirectory('second');second.children.set('main.tex',new MemoryFile('main.tex','second'));
 const old=await readProject(first.asHandle());old.files[0].text='unsaved';const next=await readProject(second.asHandle());
 assert.notEqual(old.id,next.id);assert.equal(old.files[0].text,'unsaved');assert.equal(next.files[0].text,'second');assert.equal(dirtyFiles(next).length,0);
});
...
test('project build diagnostics restore result, engine, timestamp and source position',async()=>{
 const directory=new MemoryDirectory('paper');directory.children.set('main.tex',new MemoryFile('main.tex','text'));
 const record={items:[{id:'one',severity:'warning' as const,message:'warning',path:'main.tex',line:1}],signature:'fingerprint',rootId:'main.tex',status:'success' as const,log:'actual log',engine:'pdflatex',timestamp:12345};
 await persistDiagnostics(directory.asHandle(),record);const project=await readProject(directory.asHandle());assert.deepEqual(project.diagnostics,record);assert.equal(project.compileLog,'actual log');assert(!projectTree(project.files,project.directories).some(n=>n.name==='build'));
});

test('project preferences persist, restore inheritance and stay isolated from another project',async()=>{
 const a=new MemoryDirectory('a'),b=new MemoryDirectory('b');for(const root of [a,b]){root.children.set('main.tex',new MemoryFile('main.tex','text'));root.children.set('paperdesk.json',new MemoryFile('paperdesk.json',JSON.stringify({main:'main.tex',engine:'pdflatex',name:root.name})));}
 const original=await readProject(a.asHandle());assert.equal(original.settings!.overrides.engine,'pdflatex');
 const changed=await saveProjectConfiguration(original,{version:1,overrides:{engine:'xelatex',lintEnabled:false,disabledRules:[26]}});
 assert.equal((await readProject(a.asHandle())).settings!.overrides.engine,'xelatex');assert.equal((await readProject(b.asHandle())).settings!.overrides.engine,'pdflatex');
 await saveProjectConfiguration(changed,{version:1,overrides:{}});const restored=await readProject(a.asHandle());assert.deepEqual(restored.settings!.overrides,{});assert.equal(restored.engine,undefined);assert.equal(JSON.parse(restored.files.find(f=>f.path==='paperdesk.json')!.text!).name,'a');
});
...
test('project settings write to isolated disk directories and reject external conflicts',async()=>{
 ...
 try{
  ...
  assert.deepEqual(JSON.parse(await fs.readFile(join(temporary,'a','build','preview.json'),'utf8')),{version:1});
...
test('build output is scoped to the selected project and denied diagnostics are not reported saved',async()=>{
 const a=new MemoryDirectory('paper-a'),b=new MemoryDirectory('paper-b');
 await persistBuild(a.asHandle(),new File(['%PDF-output'],'main.pdf'),'actual log','{"version":1}');
...
test('immediate save uses latest buffer once, preserves edits during write and reports failures',async()=>{
 const {createProjectSaver}=await import('../src/lib/projectSaver');const root=new MemoryDirectory('paper');const file=new MemoryFile('notes.md','original');root.children.set('notes.md',file);let project=await readProject(root.asHandle()),status='';let writes=0,finish:()=>void=()=>{};
 file.createWritable=async()=>{writes++;let value='';return {write:async text=>{value=text;},close:()=>new Promise<void>(resolve=>{finish=()=>{file.contents=value;resolve();};}),abort:async()=>{}};};
 const save=createProjectSaver({getProject:()=>project,setProject:fn=>{project=fn(project);},message:value=>{status=value;},saving:()=>{}});
