...
function ready(router:ReturnType<typeof createMemoryRouter>,pathname:string){
 if(router.state.initialized&&router.state.navigation.state==='idle'&&router.state.location.pathname===pathname)return Promise.resolve();
 return new Promise<void>((resolve,reject)=>{const timer=setTimeout(()=>{unsubscribe();reject(Error(`Navigation did not settle at ${pathname}`));},2000);const unsubscribe=router.subscribe(state=>{if(state.initialized&&state.navigation.state==='idle'&&state.location.pathname===pathname){clearTimeout(timer);unsubscribe();resolve();}});});
}
...
test('refresh-equivalent direct entry does not depend on previous view state',async()=>{
 for(const path of Object.values(viewPaths)){const router=routerAt(path);try{await ready(router,resolvePage(path).redirect??path);assert.equal(router.state.loaderData['0'],resolvePage(path).view);}finally{router.dispose();}}
});

test('settings scope/category pages support direct entry and history',async()=>{
 const paths=['/settings/global/general','/settings/global/editor','/settings/global/compile','/settings/global/references','/settings/project/general','/settings/project/editor','/settings/project/compile','/settings/project/references'];
 for(const path of paths){const router=routerAt(path);try{await ready(router,path);assert.equal(router.state.loaderData['0'],'settings');}finally{router.dispose();}}
 const router=routerAt(paths[1]);try{await ready(router,paths[1]);await router.navigate(paths[6]);await ready(router,paths[6]);await router.navigate(-1);await ready(router,paths[1]);}finally{router.dispose();}
});
