...
async function searchScholar(query, limit) {
  ...
  return (data.data ?? []).map(p => ({
    title: p.title,
    authors: (p.authors ?? []).map(a => a.name).slice(0, 6).join(", "),
...
async function searchCrossref(query, limit) {
  ...
  return (data.message?.items ?? []).map(w => ({
    title: w.title?.[0],
    authors: (w.author ?? []).map(a => [a.given, a.family].filter(Boolean).join(" ")).slice(0, 6).join(", "),
    year: w.issued?.["date-parts"]?.[0]?.[0], venue: w["container-title"]?.[0],
    doi: w.DOI, url: w.URL,
  }));
}
...
async function searchBrave(query, limit) {
  ...
  const data = await response.json();
  return (data.web?.results ?? []).map(r => ({ title: r.title, url: r.url, description: r.description, age: r.age }));
}
...
export default function (pi) {
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      source: Type.Optional(Type.Union([Type.Literal("auto"), Type.Literal("scholar"), Type.Literal("web")], { description: "auto: papers first, web fallback; scholar: academic only; web: Brave only" })),
    ...
    async execute(_toolCallId, params, signal) {
      ...
      try {
        ...
        if (source === "web") {
          ...
          try { papers = await searchScholar(params.query, limit); }
        ...
        return { content: [{ type: "text", text: `web_search 失败:${error.message}` }], details: { error: error.message } };
      }
    },
  });
}
