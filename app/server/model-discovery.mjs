...
export const discoveryAdapters={
 opencode:{url:'https://opencode.ai/zen/v1/models',baseUrl:'https://opencode.ai/zen/v1',anthropicBaseUrl:'https://opencode.ai/zen',public:true},
 'opencode-go':{url:'https://opencode.ai/zen/go/v1/models',baseUrl:'https://opencode.ai/zen/go/v1',anthropicBaseUrl:'https://opencode.ai/zen/go',public:true},
...
export function createModelDiscovery({directory,fetcher=fetch,now=Date.now,ttl=300000}={}){
 ...
 const cached=memory.get(cacheKey);if(cached&&!force&&now()-cached.at<ttl)return {...cached.result,state:cached.result.state==='live'?'cache':cached.result.state};
 ...
 if(!force&&!cached&&disk?.checkedAt&&now()-disk.checkedAt<ttl){memory.set(cacheKey,{at:now(),result:disk});return {...disk,state:'cache'};}
