...
export function researchTools(initialRoot) {
  ...
  const coding = createCodingTools(initialRoot).map((tool) => ({
    ...tool,
    async execute(...args) {
  ...
  const workspaces = {
    ...
    async execute(_id, input) {
      ...
      if (input.action === "list") result = await listWorkspaces(initialRoot)
        ...
        selected = await workspaceTarget(initialRoot, input.target)
        result = {
          selected,
          note: "Subsequent tools in this task use this directory. The visible editor is unchanged.",
        }
      } else if (input.action === "results") result = await listWorkspaceResults(initialRoot)
      else if (input.action === "save")
        result = await saveWorkspaceResult(initialRoot, { ...input, source: selected })
      else throw Error("Unknown workspace action")
      return {
        content: [{ type: "text", text: JSON.stringify({ selected, result }) }],
        details: { selected },
      }
    },
  }
