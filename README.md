...
Requires Node.js 24+ and npm. From this repository root:
...
Desktop compilation supports Windows and macOS with an installed TeX toolchain. Trust a workspace once to enable Git, LaTeX and AI together. System and CPU architecture, Git, TeX and optional tools are detected automatically from PATH and common installation directories; `ENVOI_TEX_BIN` can select a TeX binary directory. Git status uses the detected Git executable; editor checks auto-detect ChkTeX and support a validated custom executable path in settings. The application reports missing tools and does not install them automatically. Static hosting does not provide desktop capabilities.
