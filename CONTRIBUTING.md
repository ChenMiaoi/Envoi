...
`npm run lint` runs the repository ESLint configuration and must pass cleanly. CI (`.github/workflows/ci.yml`) runs on Windows and macOS and enforces lint, build, `npm test`, `test:ai`, `test:local`, and `test:desktop` on every push and pull request; without a TeX toolchain on the runner, TeX-dependent local tests self-skip. Native compiler tests exercise PDFLaTeX and XeLaTeX on both platforms; legacy sandbox tests remain macOS-only. Windows file-symlink tests skip when Developer Mode/elevation is unavailable. `test:demo` (which needs a compiled example PDF from `demo:build`) remains a per-machine step.
